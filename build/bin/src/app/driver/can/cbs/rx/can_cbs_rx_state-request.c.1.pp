...
static void CANRX_ClearAllPersistentFlags(uint64_t signalData) {
    ...
    for (uint8_t i = 0; i < 11u; i++) {
        persistant_error_flags[i].latched_value = 0;
    }
}

static void CANRX_HandleModeRequest(uint64_t signalData, const CAN_SHIM_s *const kpkCanShim) {
    do { if (!(kpkCanShim != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 133); } while (0); FAS_InfiniteLoop(); } } while (0);
    ...
    if ((kpkCanShim->pTableStateRequest->stateRequestViaCan !=
        ...
        (OS_CheckTimeHasPassed(kpkCanShim->pTableStateRequest->header.timestamp, (3000u)))) {
        kpkCanShim->pTableStateRequest->stateRequestViaCanPending = stateRequest;
    }
    if (kpkCanShim->pTableStateRequest->stateCounter == (uint8_t)0xff) {
...
static void CANRX_SetBalancingThreshold(uint64_t signalData) {
    ...
    if (signalData > (uint64_t)0xffff) {
        cappedSignalData = (uint64_t)0xffff;
    }
...
extern uint32_t CANRX_BmsStateRequest(
    ...
    const uint8_t *const kpkCanData,
    const CAN_SHIM_s *const kpkCanShim) {
    do { if (!(message . id == (0x230u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 197); } while (0); FAS_InfiniteLoop(); } } while (0);
    ...
    do { if (!(kpkCanData != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 200); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(kpkCanShim != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 201); } while (0); FAS_InfiniteLoop(); } } while (0);

    (DATA_Read1DataBlock) (kpkCanShim->pTableStateRequest);
    ...
    (DATA_Write1DataBlock) (kpkCanShim->pTableStateRequest);
