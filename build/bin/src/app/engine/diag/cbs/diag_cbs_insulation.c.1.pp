...
extern void DIAG_Insulation(
    ...
    if (diagId == DIAG_ID_INSULATION_MEASUREMENT_VALID) {
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->insulationMeasurementInvalidError = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->insulationMeasurementInvalidError = 1;
        }
    } else if (diagId == DIAG_ID_LOW_INSULATION_RESISTANCE_ERROR) {
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->criticalLowInsulationResistanceError = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->criticalLowInsulationResistanceError = 1;
        }
    } else if (diagId == DIAG_ID_LOW_INSULATION_RESISTANCE_WARNING) {
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->warnableLowInsulationResistanceError = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->warnableLowInsulationResistanceError = 1;
        }
    } else if (diagId == DIAG_ID_INSULATION_GROUND_ERROR) {
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->insulationGroundFaultDetectedError = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->insulationGroundFaultDetectedError = 1;
        }
    }
}
