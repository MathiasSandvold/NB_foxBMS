...
extern void DIAG_AlertFlag(
    ...
    const DIAG_DATABASE_SHIM_s *const kpkDiagShim,
    uint32_t data) {
    do { if (!(diagId == DIAG_ID_ALERT_MODE)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 78); } while (0); FAS_InfiniteLoop(); } } while (0);
    ...
    if (event == DIAG_EVENT_RESET) {
        kpkDiagShim->pTableError->alertFlagSetError = 0;
    }
    if (event == DIAG_EVENT_NOT_OK) {
        kpkDiagShim->pTableError->alertFlagSetError = 1;
    }
}
...
extern void DIAG_PrechargeProcess(
    ...
    const DIAG_DATABASE_SHIM_s *const kpkDiagShim,
    uint32_t stringNumber) {
    do { if (!((diagId == DIAG_ID_PRECHARGE_ABORT_REASON_VOLTAGE) || (diagId == DIAG_ID_PRECHARGE_ABORT_REASON_CURRENT))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 98); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!((event == DIAG_EVENT_OK) || (event == DIAG_EVENT_NOT_OK) || (event == DIAG_EVENT_RESET))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 99); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(kpkDiagShim != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 100); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(stringNumber < (1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 101); } while (0); FAS_InfiniteLoop(); } } while (0);

    if (diagId == DIAG_ID_PRECHARGE_ABORT_REASON_VOLTAGE) {
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->prechargeAbortedDueToVoltage[stringNumber] = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->prechargeAbortedDueToVoltage[stringNumber] = 1;
        }
    } else if (diagId == DIAG_ID_PRECHARGE_ABORT_REASON_CURRENT) {
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->prechargeAbortedDueToCurrent[stringNumber] = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->prechargeAbortedDueToCurrent[stringNumber] = 1;
        }
    } else {
