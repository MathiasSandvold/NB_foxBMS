...
extern STD_RETURN_TYPE_e PL_CheckCellVoltage(
    ...
    do { if (!(pCellVoltage != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 95); } while (0); FAS_InfiniteLoop(); } } while (0);
    STD_RETURN_TYPE_e retval = STD_OK;

    if (abs(baseCellVoltage - redundancy0CellVoltage) > (10)) {
        retval = STD_NOT_OK;
    }
...
extern STD_RETURN_TYPE_e PL_CheckCelltemperature(
    ...
    do { if (!(pCelltemperature != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 113); } while (0); FAS_InfiniteLoop(); } } while (0);

    STD_RETURN_TYPE_e retval = STD_OK;

    if (abs(baseCelltemperature - redundancy0Celltemperature) > (50)) {
        retval = STD_NOT_OK;
    }
...
extern STD_RETURN_TYPE_e PL_CheckVoltageSpread(
    ...
    do { if (!(pCellVoltages != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 129); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pMinMaxAverageValues != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 130); } while (0); FAS_InfiniteLoop(); } } while (0);
        ...
        for (uint8_t m = 0u; m < (3u); m++) {
            for (uint8_t c = 0u; c < (16u); c++) {
                ...
                if ((pCellVoltages->invalidCellVoltage[s][m] & (0x01u << c)) == 0) {
                    if (abs(pCellVoltages->cellVoltage_mV[s][(m * (16u)) + c] -
                            pMinMaxAverageValues->averageCellVoltage_mV[s]) > (450)) {
...
extern STD_RETURN_TYPE_e PL_CheckTemperatureSpread(
    ...
    do { if (!(pCellTemperatures != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 161); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pMinMaxAverageValues != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 162); } while (0); FAS_InfiniteLoop(); } } while (0);
        ...
        for (uint8_t m = 0u; m < (3u); m++) {
            for (uint8_t c = 0u; c < (8u); c++) {
                ...
                if ((pCellTemperatures->invalidCellTemperature[s][m] & (0x01u << c)) == 0) {
                    if (abs(pCellTemperatures->cellTemperature_ddegC[s][(m * (8u)) + c] -
