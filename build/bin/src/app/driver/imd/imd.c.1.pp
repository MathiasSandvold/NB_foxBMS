...
typedef struct {
    _Bool isStatemachineInitialized;  
    _Bool switchImdDeviceOn;          
} IMD_INFORMATION_s;
...
static IMD_STATE_s imd_state = {
    ...
    .information.isStatemachineInitialized = 0,
    .information.switchImdDeviceOn         = 0,
...
static IMD_RETURN_TYPE_e IMD_SetStateRequest(IMD_STATE_s *pImdState, IMD_STATE_REQUEST_e stateRequest) {
    do { if (!(pImdState != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 237); } while (0); FAS_InfiniteLoop(); } } while (0);
...
static IMD_RETURN_TYPE_e IMD_CheckStateRequest(IMD_STATE_s *pImdState, IMD_STATE_REQUEST_e stateRequest) {
    do { if (!(pImdState != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 253); } while (0); FAS_InfiniteLoop(); } } while (0);
...
static IMD_STATE_REQUEST_e IMD_TransferStateRequest(IMD_STATE_s *pImdState) {
    do { if (!(pImdState != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 282); } while (0); FAS_InfiniteLoop(); } } while (0);
    OS_EnterTaskCritical();
...
static IMD_CHECK_MULTIPLE_CALLS_e IMD_CheckMultipleCalls(IMD_STATE_s *pImdState) {
    do { if (!(pImdState != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 291); } while (0); FAS_InfiniteLoop(); } } while (0);
    IMD_CHECK_MULTIPLE_CALLS_e multipleCalls = IMD_MULTIPLE_CALLS_NO;
    OS_EnterTaskCritical();
    if (pImdState->triggerEntry == 0u) {
        pImdState->triggerEntry++;
    } else {
        multipleCalls = IMD_MULTIPLE_CALLS_YES;  
    }
    OS_ExitTaskCritical();
...
static void IMD_SetState(
    ...
    IMD_FSM_SUBSTATES_e nextSubstate,
    uint16_t idleTime) {
    do { if (!(pImdState != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 308); } while (0); FAS_InfiniteLoop(); } } while (0);
    ...
    if ((pImdState->currentState == nextState) && (pImdState->currentSubstate == nextSubstate)) {
        ...
        pImdState->nextState    = IMD_FSM_STATE_DUMMY;     
        pImdState->nextSubstate = IMD_FSM_SUBSTATE_DUMMY;  
        earlyExit               = 1;
    }
    ...
    if (earlyExit == 0) {
        if (pImdState->currentState != nextState) {
            ...
            pImdState->currentSubstate  = IMD_FSM_SUBSTATE_ENTRY;  
            pImdState->nextState        = IMD_FSM_STATE_DUMMY;     
            pImdState->nextSubstate     = IMD_FSM_SUBSTATE_DUMMY;  
...
static void IMD_SetSubstate(IMD_STATE_s *pImdState, IMD_FSM_SUBSTATES_e nextSubstate, uint16_t idleTime) {
    do { if (!(pImdState != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 342); } while (0); FAS_InfiniteLoop(); } } while (0);
...
static STD_RETURN_TYPE_e IMD_RunStateMachine(IMD_STATE_s *pImdState) {
    do { if (!(pImdState != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 352); } while (0); FAS_InfiniteLoop(); } } while (0);
    ...
    switch (pImdState->currentState) {
        ...
        case IMD_FSM_STATE_HAS_NEVER_RUN:
            ...
            IMD_SetState(pImdState, IMD_FSM_STATE_UNINITIALIZED, IMD_FSM_SUBSTATE_ENTRY, (1u));
            break;
        ...
        case IMD_FSM_STATE_UNINITIALIZED:
            ...
            if (stateRequest == IMD_STATE_INITIALIZE_REQUEST) {
                IMD_SetState(pImdState, IMD_FSM_STATE_INITIALIZATION, IMD_FSM_SUBSTATE_ENTRY, (1u));
            } else {
        ...
        case IMD_FSM_STATE_INITIALIZATION:
            ...
            if (nextState == IMD_FSM_STATE_INITIALIZATION) {
                ...
                IMD_SetState(pImdState, IMD_FSM_STATE_ERROR, IMD_FSM_SUBSTATE_ENTRY, (1u));
            } else if (nextState == IMD_FSM_STATE_IMD_ENABLE) {
                pImdState->information.isStatemachineInitialized = 1;
                IMD_SetState(pImdState, IMD_FSM_STATE_IMD_ENABLE, IMD_FSM_SUBSTATE_ENTRY, (1u));
            } else {
                do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 385); } while (0); FAS_InfiniteLoop(); } } while (0);  
            }
            break;
        ...
        case IMD_FSM_STATE_IMD_ENABLE:
            ...
            if (stateRequest == IMD_STATE_SWITCH_ON_REQUEST) {
                pImdState->information.switchImdDeviceOn = 1;
            }
            if (pImdState->information.switchImdDeviceOn == 1) {
                nextState = IMD_ProcessEnableState();
                if (nextState == IMD_FSM_STATE_RUNNING) {
                    
 
                    pImdState->pTableImd->isImdRunning       = 1;
                    pImdState->information.switchImdDeviceOn = 0;
                    IMD_SetState(pImdState, IMD_FSM_STATE_RUNNING, IMD_FSM_SUBSTATE_ENTRY, (1u));
                } else if (nextState == IMD_FSM_STATE_ERROR) {
                    IMD_SetState(pImdState, IMD_FSM_STATE_ERROR, IMD_FSM_SUBSTATE_ENTRY, (1u));
                } else {
        ...
        case IMD_FSM_STATE_RUNNING:
            ...
            if (stateRequest == IMD_STATE_SHUTDOWN_REQUEST) {
                IMD_SetState(pImdState, IMD_FSM_STATE_SHUTDOWN, IMD_FSM_SUBSTATE_ENTRY, (1u));
            } else {
        ...
        case IMD_FSM_STATE_SHUTDOWN:
            ...
            if (nextState == IMD_FSM_STATE_IMD_ENABLE) {
                ...
                pImdState->pTableImd->isImdRunning = 0;
                (void)(DATA_Write1DataBlock) (&imd_tableInsulationMonitoring);
                IMD_SetState(pImdState, IMD_FSM_STATE_IMD_ENABLE, IMD_FSM_SUBSTATE_ENTRY, (1u));
            } else if (nextState == IMD_FSM_STATE_ERROR) {
                IMD_SetState(pImdState, IMD_FSM_STATE_ERROR, IMD_FSM_SUBSTATE_ENTRY, (1u));
            } else {
        ...
        case IMD_FSM_STATE_ERROR:
            ...
            do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 447); } while (0); FAS_InfiniteLoop(); } } while (0);
            break;

         
        default:
             
            do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 453); } while (0); FAS_InfiniteLoop(); } } while (0);
            break;
    }
...
static STD_RETURN_TYPE_e IMD_EvaluateInsulationMeasurement(
    DATA_BLOCK_INSULATION_MONITORING_s *pTableInsulationMonitoring) {
    do { if (!(pTableInsulationMonitoring != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 463); } while (0); FAS_InfiniteLoop(); } } while (0);
     
    _Bool lowResistanceDetected = 0;

     
    if (pTableInsulationMonitoring->isInsulationMeasurementValid == 1) {
        (void)DIAG_Handler(DIAG_ID_INSULATION_MEASUREMENT_VALID, DIAG_EVENT_OK, DIAG_SYSTEM, 0u);
        if (pTableInsulationMonitoring->insulationResistance_kOhm < (500u)) {
            lowResistanceDetected = 1;
        }
    } else {
    ...
    if (pTableInsulationMonitoring->areDeviceFlagsValid == 1) {
        if (pTableInsulationMonitoring->dfIsCriticalResistanceDetected == 1) {
            lowResistanceDetected = 1;
        }
        if (pTableInsulationMonitoring->dfIsChassisFaultDetected == 1) {
    ...
    if (pTableInsulationMonitoring->dfIsWarnableResistanceDetected == 1) {
    ...
    (DATA_Write1DataBlock) (pTableInsulationMonitoring);
