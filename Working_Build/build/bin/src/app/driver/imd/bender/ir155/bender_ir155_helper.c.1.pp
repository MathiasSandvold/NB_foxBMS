...
typedef struct {
    _Bool isMeasurementValid;                     
    _Bool isUndervoltageDetected;                 
...
extern IR155_STATE_s ir155_state = {
    .ir155Initialized                     = 0,
    .measurement.isMeasurementValid       = 0,
    .measurement.isUndervoltageDetected   = 0,
...
static _Bool IR155_IsDutyCycleWithinInterval(float_t dutyCycle_perc, float_t lowerLimit_perc, float_t upperLimit_perc);

 
static IR155_MEASUREMENT_MODE_e IR155_GetMeasurementMode(float_t frequency_Hz) {
    do { if (!(frequency_Hz >= 0.0f)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 142); } while (0); FAS_InfiniteLoop(); } } while (0);
    IR155_MEASUREMENT_MODE_e retVal = IR155_UNKNOWN;

    if ((frequency_Hz >= ((10.0f) - (1.0f))) &&
        (frequency_Hz < ((10.0f) + (1.0f)))) {
        retVal = IR155_NORMAL_MODE;
    } else if (
        (frequency_Hz >= ((20.0f) - (1.0f))) &&
        (frequency_Hz < ((20.0f) + (1.0f)))) {
        retVal = IR155_UNDERVOLTAGE_MODE;  
    } else if (
        (frequency_Hz >= ((30.0f) - (1.0f))) && (frequency_Hz < ((30.0f) + (1.0f)))) {
        retVal = IR155_SPEEDSTART_MODE;
    } else if (
        (frequency_Hz >= ((40.0f) - (1.0f))) &&
        (frequency_Hz < ((40.0f) + (1.0f)))) {
        retVal = IR155_IMD_ERROR_MODE;
    } else if (
        (frequency_Hz >= ((50.0f) - (1.0f))) &&
        (frequency_Hz < ((50.0f) + (1.0f)))) {
        retVal = IR155_GROUNDERROR_MODE;
    } else if (frequency_Hz <= (5.0f)) {
...
static uint32_t IR155_CalculateResistance(float_t dutyCycle_perc) {
    do { if (!(dutyCycle_perc <= 100.0f)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 172); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(dutyCycle_perc > 0.0f)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 173); } while (0); FAS_InfiniteLoop(); } } while (0);

    float_t insulationResistance_kOhm = 0.0f;
    if (dutyCycle_perc <= 5.0f) {
        insulationResistance_kOhm = (float_t)(106800u);
    } else if (dutyCycle_perc > 95.0f) {
        insulationResistance_kOhm = (float_t)(0u);
    } else {
...
void IR155_Initialize(uint8_t triggerTime_ms) {
    ...
    if (0 == PWM_IsEcapModuleInitialized()) {
        PWM_Initialize();  
    }
    ...
    if (fram_insulationFlags.groundErrorDetected == 1) {
         
        ir155_state.timeUntilValidMeasurement_ms = (25000u);
    } else {
         
        ir155_state.timeUntilValidMeasurement_ms = (2200u);
    }

    ir155_state.periodTriggerTime_ms = triggerTime_ms;
    ir155_state.ir155Initialized     = 1;
}
...
void IR155_Deinitialize(void) {
    ...
    ir155_state.ir155Initialized = 0;
...
IR155_MEASUREMENT_s IR155_GetMeasurementValues(void) {
    ...
    IR155_MEASUREMENT_s measurementResult = {
        .isMeasurementValid       = 0,
        .isUndervoltageDetected   = 1,
    ...
    measurementResult.digitalStatusPin = IO_PinGet(&(((hetBASE_t *)0xFFF7B800U))->DIN, (27u));

     
    measurementResult.pwmSignal = PWM_GetPwmData();

     
    _Bool isPwmMeasurementValid = 1;
    if (isPwmMeasurementValid == 1) {
         
        isPwmMeasurementValid = 1;
    } else {
         

         
        isPwmMeasurementValid = 0;
    }
    ...
    switch (measurementResult.measurementMode) {
        case IR155_NORMAL_MODE:
            ir155_state.measurement.isUndervoltageDetected = 0;
            if (1 == IR155_IsDutyCycleWithinInterval(
                ...
                measurementResult.isMeasurementValid = 1;
                ...
                measurementResult.isMeasurementValid = 0;
                measurementResult.resistance_kOhm    = (0u);
            }
            break;

        case IR155_SPEEDSTART_MODE:
            measurementResult.isUndervoltageDetected = 0;
            if (1 == IR155_IsDutyCycleWithinInterval(
                            measurementResult.pwmSignal.dutyCycle_perc,
                            (4.0f),
                            (11.0f))) {
                measurementResult.resistance_kOhm    = (106800u);
                measurementResult.isMeasurementValid = 1;
                measurementResult.measurementState   = IR155_RESISTANCE_ESTIMATION;
            } else if (
                1 == IR155_IsDutyCycleWithinInterval(
                            measurementResult.pwmSignal.dutyCycle_perc,
                            (89.0f),
                            (96.0f))) {
                measurementResult.resistance_kOhm    = (0u);
        ...
        case IR155_UNDERVOLTAGE_MODE:
            measurementResult.isUndervoltageDetected = 1;
            if (1 == IR155_IsDutyCycleWithinInterval(
                            measurementResult.pwmSignal.dutyCycle_perc,
                            (4.0f),
                            (96.0f))) {
