...
extern void DIAG_ErrorAfe(
    ...
    if (diagId == DIAG_ID_AFE_CELL_VOLTAGE_MEAS_ERROR) {
        ...
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->afeCellTemperatureInvalidError[stringNumber] = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->afeCellTemperatureInvalidError[stringNumber] = 1;
        }
    } else if (diagId == DIAG_ID_BASE_CELL_VOLTAGE_MEASUREMENT_TIMEOUT) {
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->baseCellVoltageMeasurementTimeoutError = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->baseCellVoltageMeasurementTimeoutError = 1;
        }
    } else if (diagId == DIAG_ID_REDUNDANCY0_CELL_VOLTAGE_MEASUREMENT_TIMEOUT) {
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->redundancy0CellVoltageMeasurementTimeoutError = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->redundancy0CellVoltageMeasurementTimeoutError = 1;
        }
    } else if (diagId == DIAG_ID_BASE_CELL_TEMPERATURE_MEASUREMENT_TIMEOUT) {
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->baseCellTemperatureMeasurementTimeoutError = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->baseCellTemperatureMeasurementTimeoutError = 1;
        }
    } else if (diagId == DIAG_ID_REDUNDANCY0_CELL_TEMPERATURE_MEASUREMENT_TIMEOUT) {
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->redundancy0CellTemperatureMeasurementTimeoutError = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->redundancy0CellTemperatureMeasurementTimeoutError = 1;
        }
    }
}
...
extern void DIAG_ErrorAfeDriver(
    ...
    const DIAG_DATABASE_SHIM_s *const kpkDiagShim,
    uint32_t stringNumber) {
    do { if (!(diagId < DIAG_ID_MAX)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 132); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!((event == DIAG_EVENT_OK) || (event == DIAG_EVENT_NOT_OK) || (event == DIAG_EVENT_RESET))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 133); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(kpkDiagShim != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 134); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(stringNumber < (1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 135); } while (0); FAS_InfiniteLoop(); } } while (0);

    if (diagId == DIAG_ID_AFE_SPI) {
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->afeCommunicationSpiError[stringNumber] = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->afeCommunicationSpiError[stringNumber] = 1;
        }
    } else if (diagId == DIAG_ID_AFE_COM_INTEGRITY) {
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->afeCommunicationCrcError[stringNumber] = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->afeCommunicationCrcError[stringNumber] = 1;
        }
    } else if (diagId == DIAG_ID_AFE_MUX) {
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->afeSlaveMultiplexerError[stringNumber] = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->afeSlaveMultiplexerError[stringNumber] = 1;
        }
        ...
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->openWireDetectedError[stringNumber] = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->openWireDetectedError[stringNumber] = 1;
        }
    } else {  
        do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 173); } while (0); FAS_InfiniteLoop(); } } while (0);
    }
}
