...
static adcData_t adc_adc1RawVoltages[(32u)] = {0};
...
static float_t ADC_ConvertVoltage(uint16_t adcCounts) {
    ...
    float_t result_mV = (((adcCounts + (0.5f)) * ((5000.0f) - (0.0f))) / (4096.0f)) +
                        (0.0f);
...
extern void ADC_Control(void) {
    ...
    switch (adc_conversionState) {
        ...
        case ADC_WAIT_CONVERSION_FINISHED:
            ...
            if ((8u) != adcIsConversionComplete(((adcBASE_t *)0xFFF7C000U), 1U)) {
                conversionFinished = 0;
            }
            if (conversionFinished == 1) {
        ...
        case ADC_CONVERSION_FINISHED:
            adcGetData(((adcBASE_t *)0xFFF7C000U), 1U, &adc_adc1RawVoltages[0]);
            for (uint8_t i = 0u; i < (32u); i++) {
                adc_adc1Voltages.adc1ConvertedVoltages_mV[i] = ADC_ConvertVoltage(adc_adc1RawVoltages[i].value);
            }
            (DATA_Write1DataBlock) (&adc_adc1Voltages);
            adc_conversionState = ADC_START_CONVERSION;
            break;

        default:
             
            do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 135); } while (0); FAS_InfiniteLoop(); } } while (0);
            break;
    }
}
