...
extern _Bool SOA_IsPackCurrentLimitViolated(uint32_t current_mA, BMS_CURRENT_FLOW_STATE_e currentDirection);
...
extern _Bool SOA_IsStringCurrentLimitViolated(uint32_t current_mA, BMS_CURRENT_FLOW_STATE_e currentDirection);
...
extern _Bool SOA_IsCellCurrentLimitViolated(uint32_t current_mA, BMS_CURRENT_FLOW_STATE_e currentDirection);
...
extern _Bool SOA_IsCurrentOnOpenString(BMS_CURRENT_FLOW_STATE_e currentDirection, uint8_t stringNumber);
...
static BMS_STATE_s bms_state = {
    ...
    .restTimer_10ms                    = (60000u),
    .currentFlowState                  = BMS_RELAXATION,
    .remainingDelay_ms                 = (0xffffffffU),
    .minimumActiveDelay_ms             = (0xffffffffU),
    .transitionToErrorState            = 0,
...
static _Bool BMS_IsAnyFatalErrorFlagSet(void);
...
static _Bool BMS_IsContactorFeedbackValid(uint8_t stringNumber, CONT_TYPE_e contactorType);
...
static void BMS_GetMeasurementValues(void) {
    (DATA_Read3DataBlocks) (&bms_tablePackValues, &bms_tableOpenWire, &bms_tableMinMax);
}
...
static uint8_t BMS_CheckCanRequests(void) {
    ...
    (DATA_Read1DataBlock) (&request);

    if (request.stateRequestViaCan == (3u)) {
        retVal = (3u);
    } else if (request.stateRequestViaCan == (1u)) {
        retVal = (1u);
    } else if (request.stateRequestViaCan == (2u)) {
        retVal = (2u);
    } else if (request.stateRequestViaCan == (0u)) {
        retVal = (0u);
    } else {
...
static void BMS_CheckOpenSenseWire(void) {
    ...
    for (uint8_t s = 0u; s < (1u); s++) {
        ...
        for (uint8_t m = 0u; m < (3u); m++) {
            ...
            for (uint8_t wire = 0u; wire < ((16u) + 1); wire++) {
                 
                if (bms_tableOpenWire.openWire[s][(wire + (m * ((16u) + 1))) == 1] > 0u) {
                    openWireDetected++;
...
static STD_RETURN_TYPE_e BMS_CheckPrecharge(uint8_t stringNumber, const DATA_BLOCK_PACK_VALUES_s *pPackValues) {
    ...
    do { if (!(stringNumber < (1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 410); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pPackValues != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 411); } while (0); FAS_InfiniteLoop(); } } while (0);
...
static STD_RETURN_TYPE_e BMS_IsBatterySystemStateOkay(void) {
    ...
    const _Bool isErrorActive = BMS_IsAnyFatalErrorFlagSet();

     
    if (bms_state.transitionToErrorState == 1) {
        ...
        if (isErrorActive == 1) {
            bms_state.transitionToErrorState = 1;
            bms_state.remainingDelay_ms      = bms_state.minimumActiveDelay_ms;
        }
    }
    ...
    if ((bms_state.transitionToErrorState == 1) && (bms_state.remainingDelay_ms == 0u)) {
        retVal = STD_NOT_OK;
    }
...
static _Bool BMS_IsContactorFeedbackValid(uint8_t stringNumber, CONT_TYPE_e contactorType) {
    do { if (!(stringNumber < (1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 498); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(contactorType != CONT_UNDEFINED)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 499); } while (0); FAS_InfiniteLoop(); } } while (0);
    _Bool feedbackValid = 0;
     
    DATA_BLOCK_ERROR_STATE_s tableErrorFlags = {.header.uniqueId = DATA_BLOCK_ID_ERROR_STATE};
    (DATA_Read1DataBlock) (&tableErrorFlags);
     
    switch (contactorType) {
        case CONT_PLUS:
            if (tableErrorFlags.contactorInPositivePathOfStringFeedbackError[stringNumber] == 0) {
                feedbackValid = 1;
            }
            break;
        case CONT_MINUS:
            if (tableErrorFlags.contactorInNegativePathOfStringFeedbackError[stringNumber] == 0) {
                feedbackValid = 1;
            }
            break;
        case CONT_PRECHARGE:
            if (tableErrorFlags.prechargeContactorFeedbackError[stringNumber] == 0) {
                feedbackValid = 1;
            }
            break;
        default:
...
static uint8_t BMS_GetHighestString(BMS_CONSIDER_PRECHARGE_e precharge, DATA_BLOCK_PACK_VALUES_s *pPackValues) {
    do { if (!(pPackValues != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 529); } while (0); FAS_InfiniteLoop(); } } while (0);
    uint8_t highest_string_index = (255u);
    int32_t max_stringVoltage_mV = (-0x7fffffff-1);
...
static uint8_t BMS_GetClosestString(BMS_CONSIDER_PRECHARGE_e precharge, DATA_BLOCK_PACK_VALUES_s *pPackValues) {
    do { if (!(pPackValues != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 554); } while (0); FAS_InfiniteLoop(); } } while (0);
    uint8_t closestStringIndex     = (255u);
    int32_t closedStringVoltage_mV = 0;
    _Bool searchString              = 0;
    ...
    if (searchString == 1) {
        for (uint8_t s = 0u; s < (1u); s++) {
            const _Bool isStringClosed          = BMS_IsStringClosed(s);
            const uint8_t isStringVoltageValid = pPackValues->invalidStringVoltage[s];
            if ((isStringClosed == 0) && (isStringVoltageValid == 0u)) {
                 
                int32_t minimumVoltageDifference_mV = 0x7fffffff;
...
static uint8_t BMS_GetLowestString(BMS_CONSIDER_PRECHARGE_e precharge, DATA_BLOCK_PACK_VALUES_s *pPackValues) {
    do { if (!(pPackValues != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 600); } while (0); FAS_InfiniteLoop(); } } while (0);
    uint8_t lowest_string_index  = (255u);
    int32_t min_stringVoltage_mV = 0x7fffffff;
...
static int32_t BMS_GetStringVoltageDifference(uint8_t string, const DATA_BLOCK_PACK_VALUES_s *pPackValues) {
    do { if (!(string < (1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 624); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pPackValues != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 625); } while (0); FAS_InfiniteLoop(); } } while (0);
    int32_t voltageDifference_mV = 0x7fffffff;
    if ((pPackValues->invalidStringVoltage[string] == 0u) &&
        ...
        voltageDifference_mV = 0x7fffffff;
    }
    return voltageDifference_mV;
}

static int32_t BMS_GetAverageStringCurrent(DATA_BLOCK_PACK_VALUES_s *pPackValues) {
    do { if (!(pPackValues != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 644); } while (0); FAS_InfiniteLoop(); } } while (0);
    int32_t average_current = pPackValues->packCurrent_mA / (int32_t)(1u);
    if (pPackValues->invalidPackCurrent == 1u) {
        average_current = 0x7fffffff;
    }
    return average_current;
}

static void BMS_UpdateBatsysState(DATA_BLOCK_PACK_VALUES_s *pPackValues) {
    do { if (!(pPackValues != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 653); } while (0); FAS_InfiniteLoop(); } } while (0);

     
    if (pPackValues->invalidPackCurrent == 0u) {
        if ((1) == 1) {
             
            if (pPackValues->packCurrent_mA >= (200)) {  
                bms_state.currentFlowState = BMS_DISCHARGING;
                bms_state.restTimer_10ms   = (60000u);
            } else if (pPackValues->packCurrent_mA <= -(200)) {
...
static CONT_TYPE_e BMS_GetFirstContactorToBeOpened(uint8_t stringNumber, BMS_CURRENT_FLOW_STATE_e flowDirection) {
    do { if (!(stringNumber < (1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 700); } while (0); FAS_InfiniteLoop(); } } while (0);
    ...
    for (; contactor < ((2u * (1u)) + (1u)); contactor++) {
        


 
        _Bool correctString           = (_Bool)(stringNumber == cont_contactorStates[contactor].stringIndex);
        _Bool inPreferredDirection    = (_Bool)(breakingDirection == cont_contactorStates[contactor].breakingDirection);
        _Bool hasNoPreferredDirection = (_Bool)(cont_contactorStates[contactor].breakingDirection == CONT_BIDIRECTIONAL);
        _Bool noPrechargeContactor    = (_Bool)(cont_contactorStates[contactor].type != CONT_PRECHARGE);
    ...
    if (contactor == ((2u * (1u)) + (1u))) {
        

 
        for (contactor = 0u; contactor < ((2u * (1u)) + (1u)); contactor++) {
    ...
    if (contactor == ((2u * (1u)) + (1u))) {
        ...
        do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 757); } while (0); FAS_InfiniteLoop(); } } while (0);
    }
    return contactorToBeOpened;
}

static CONT_TYPE_e BMS_GetSecondContactorToBeOpened(uint8_t stringNumber, CONT_TYPE_e firstOpenedContactorType) {
    do { if (!(stringNumber < (1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 763); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!((firstOpenedContactorType != CONT_UNDEFINED) && (firstOpenedContactorType != CONT_PRECHARGE))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 764); } while (0); FAS_InfiniteLoop(); } } while (0);
    CONT_TYPE_e contactorToBeOpened = CONT_UNDEFINED;
    ...
    if (contactor == ((2u * (1u)) + (1u))) {
        ...
        do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 785); } while (0); FAS_InfiniteLoop(); } } while (0);
    }
    return contactorToBeOpened;
}
...
void BMS_Trigger(void) {
    ...
    _Bool contactorFeedbackValid                 = 0;
    ...
    switch (bms_state.state) {
        ...
        case BMS_STATEMACH_UNINITIALIZED:
            ...
            if (statereq == BMS_STATE_INIT_REQUEST) {
                bms_state . laststate = bms_state . state; bms_state . lastsubstate = bms_state . substate;
                bms_state.timer    = (1u);
        ...
        case BMS_STATEMACH_IDLE:
            ...
            if (bms_state.substate == BMS_ENTRY) {
                (DATA_Read1DataBlock) (&systemstate);
                systemstate.bmsCanState = BMS_CANSTATE_IDLE;
                (DATA_Write1DataBlock) (&systemstate);
                bms_state.timer    = (1u);
        ...
        case BMS_STATEMACH_OPEN_CONTACTORS:
            ...
            if (bms_state.substate == BMS_ENTRY) {
                ...
                if ((bms_tablePackValues.invalidStringCurrent[stringNumber] == 0u) &&
                    ...
                    bms_state.stringOpenTimeout = (1000u);
                } else {
                     
                    bms_state.timeAboveContactorBreakCurrent_ms += (10u);
                    if (bms_state.timeAboveContactorBreakCurrent_ms > (30000u)) {
                ...
                if ((contactorState == CONT_SWITCH_OFF) || (contactorFeedbackValid == 0)) {
                ...
                do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 1076); } while (0); FAS_InfiniteLoop(); } } while (0);
            }
            break;
        ...
        case BMS_STATEMACH_STANDBY:
            ...
            if (bms_state.substate == BMS_ENTRY) {
                ...
                do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 1150); } while (0); FAS_InfiniteLoop(); } } while (0);
            }
            break;
            ...
            if (bms_state.substate == BMS_ENTRY) {
                ...
                if (bms_state.OscillationTimeout == 0u) {
                    ...
                    if (CONT_CloseContactor(bms_state.firstClosedString, CONT_MINUS) == STD_OK) {
                        bms_state.stringCloseTimeout = (500u);
                        bms_state.timer              = (20u);
                ...
                if (contactorState == CONT_SWITCH_ON) {
                    bms_state.OscillationTimeout = (1000u);
                ...
                if ((contactorState == CONT_SWITCH_ON) && (retVal == STD_OK)) {
                    ...
                    if (bms_state.PrechargeTryCounter < ((3u) - 1u)) {
                        bms_state.closedPrechargeContactors[stringNumber] = 0u;
                        if (contRetVal == STD_OK) {
                            bms_state.timer    = (300u);
                            bms_state.substate = BMS_PRECHARGE_CLOSE_PRECHARGE;
                ...
                do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 1368); } while (0); FAS_InfiniteLoop(); } } while (0);
            }
            break;
        ...
        case BMS_STATEMACH_NORMAL:
            ...
            if (bms_state.substate == BMS_ENTRY) {
                ...
                if (bms_state.nextstringclosedtimer == 0u) {
                    ...
                    if (nextStringNumber == (255u)) {
                        ...
                        (BMS_GetStringVoltageDifference(nextStringNumber, &bms_tablePackValues) <=
                         (3000)) &&
                        (BMS_GetAverageStringCurrent(&bms_tablePackValues) <= (20000))) {
                         
                        CONT_CloseContactor(nextStringNumber, CONT_MINUS);
                        bms_state.nextstringclosedtimer = (500u);
                        bms_state.timer                 = (20u);
                ...
                if (contactorState == CONT_SWITCH_ON) {
                    ...
                    bms_state.nextstringclosedtimer           = (10u);
                    ...
                    bms_state.substate  = BMS_ENTRY;
                    break;
                } else if (BMS_CheckCanRequests() == (3u)) {
                    bms_state.timer     = (1u);
                    bms_state.state     = BMS_STATEMACH_OPEN_CONTACTORS;
                    bms_state.nextstate = BMS_STATEMACH_STANDBY;
                    bms_state.substate  = BMS_ENTRY;
                ...
                do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 1500); } while (0); FAS_InfiniteLoop(); } } while (0);
            }
            break;
        ...
        case BMS_STATEMACH_ERROR:
            ...
            if (bms_state.substate == BMS_ENTRY) {
                ...
                LED_SetToggleTime((100u));
                 
                nextOpenWireCheck = timestamp + (30000u);
                 
                bms_state.timer    = (1u);
                bms_state.substate = BMS_CHECK_ERROR_FLAGS;
                break;
            } else if (bms_state.substate == BMS_CHECK_ERROR_FLAGS) {
                if (DIAG_IsAnyFatalErrorSet() == 1) {
                    ...
                    bms_state.minimumActiveDelay_ms  = (0xffffffffU);
                    bms_state.minimumActiveDelay_ms  = (0xffffffffU);
                    bms_state.transitionToErrorState = 0;
                ...
                do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 1562); } while (0); FAS_InfiniteLoop(); } } while (0);
            }
            break;
        default:
             
            do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 1567); } while (0); FAS_InfiniteLoop(); } } while (0);
            break;
    }  
...
extern _Bool BMS_IsStringClosed(uint8_t stringNumber) {
    do { if (!(stringNumber < (1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 1604); } while (0); FAS_InfiniteLoop(); } } while (0);
    _Bool retval = 0;
...
extern _Bool BMS_IsStringPrecharging(uint8_t stringNumber) {
    do { if (!(stringNumber < (1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 1613); } while (0); FAS_InfiniteLoop(); } } while (0);
    _Bool retval = 0;
