...
extern void BAL_SaveLastStates(BAL_STATE_s *pBalancingState) {
    do { if (!(pBalancingState != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 74); } while (0); FAS_InfiniteLoop(); } } while (0);
...
extern uint8_t BAL_CheckReEntrance(BAL_STATE_s *currentState) {
    do { if (!(currentState != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 88); } while (0); FAS_InfiniteLoop(); } } while (0);
    uint8_t retval = 0;
...
extern BAL_STATE_REQUEST_e BAL_TransferStateRequest(BAL_STATE_s *currentState) {
    do { if (!(currentState != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 104); } while (0); FAS_InfiniteLoop(); } } while (0);
...
extern BAL_RETURN_TYPE_e BAL_CheckStateRequest(BAL_STATE_s *pCurrentState, BAL_STATE_REQUEST_e stateRequest) {
    do { if (!(pCurrentState != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 117); } while (0); FAS_InfiniteLoop(); } } while (0);
    if (stateRequest == BAL_STATE_ERROR_REQUEST) {
        return BAL_OK;
    }
    if (stateRequest == BAL_STATE_GLOBAL_ENABLE_REQUEST) {
        pCurrentState->balancingGlobalAllowed = 1;
        return BAL_OK;
    }
    if (stateRequest == BAL_STATE_GLOBAL_DISABLE_REQUEST) {
        pCurrentState->balancingGlobalAllowed = 0;
        return BAL_OK;
    }
...
extern STD_RETURN_TYPE_e BAL_Init(DATA_BLOCK_BALANCING_CONTROL_s *pControl) {
    do { if (!(pControl != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 152); } while (0); FAS_InfiniteLoop(); } } while (0);
    (DATA_Read1DataBlock) (pControl);
    pControl->enableBalancing = 0;
    (DATA_Write1DataBlock) (pControl);
    return STD_OK;
}

#pragma WEAK(BAL_ProcessStateUninitalized)
extern void BAL_ProcessStateUninitalized(BAL_STATE_s *pCurrentState, BAL_STATE_REQUEST_e stateRequest) {
    do { if (!(pCurrentState != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 161); } while (0); FAS_InfiniteLoop(); } } while (0);
    if (stateRequest == BAL_STATE_INIT_REQUEST) {
        pCurrentState->timer    = (1u);
        ...
        pCurrentState->errorRequestCounter++;  
    }
}

#pragma WEAK(BAL_ProcessStateInitialization)
extern void BAL_ProcessStateInitialization(BAL_STATE_s *currentState) {
    do { if (!(currentState != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 175); } while (0); FAS_InfiniteLoop(); } } while (0);
    currentState->timer    = (1u);
...
extern void BAL_ProcessStateInitialized(BAL_STATE_s *currentState) {
    do { if (!(currentState != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 183); } while (0); FAS_InfiniteLoop(); } } while (0);
