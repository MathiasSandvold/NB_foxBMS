...
extern void DIAG_ErrorCanTiming(
    ...
    const DIAG_DATABASE_SHIM_s *const kpkDiagShim,
    uint32_t data) {
    do { if (!(diagId == DIAG_ID_CAN_TIMING)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 78); } while (0); FAS_InfiniteLoop(); } } while (0);
    ...
    if (event == DIAG_EVENT_RESET) {
        kpkDiagShim->pTableError->stateRequestTimingViolationError = 0;
    }
    if (event == DIAG_EVENT_NOT_OK) {
        kpkDiagShim->pTableError->stateRequestTimingViolationError = 1;
    }
}
...
extern void DIAG_ErrorCanRxQueueFull(
    ...
    const DIAG_DATABASE_SHIM_s *const kpkDiagShim,
    uint32_t data) {
    do { if (!(diagId < DIAG_ID_MAX)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 95); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!((event == DIAG_EVENT_OK) || (event == DIAG_EVENT_NOT_OK) || (event == DIAG_EVENT_RESET))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 96); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(kpkDiagShim != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 97); } while (0); FAS_InfiniteLoop(); } } while (0);

    if (diagId == DIAG_ID_CAN_RX_QUEUE_FULL) {
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->canRxQueueFullError = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->canRxQueueFullError = 1;
        }
    }
}
