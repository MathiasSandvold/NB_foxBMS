...
void DIAG_ErrorDeepDischarge(
    ...
    if (diagId == DIAG_ID_DEEP_DISCHARGE_DETECTED) {
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->deepDischargeDetectedError[stringNumber] = 0;
            fram_deepDischargeFlags.deepDischargeFlag[stringNumber]            = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->deepDischargeDetectedError[stringNumber] = 1;
            fram_deepDischargeFlags.deepDischargeFlag[stringNumber]            = 1;
        }
