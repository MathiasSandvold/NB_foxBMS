...
extern void DIAG_I2c(
    ...
    const DIAG_DATABASE_SHIM_s *const kpkDiagShim,
    uint32_t data) {
    do { if (!((diagId == DIAG_ID_I2C_PEX_ERROR) || (diagId == DIAG_ID_I2C_RTC_ERROR))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 76); } while (0); FAS_InfiniteLoop(); } } while (0);
    ...
    if (diagId == DIAG_ID_I2C_PEX_ERROR) {
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->pexI2cCommunicationError = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->pexI2cCommunicationError = 1;
        }
    }

    if (diagId == DIAG_ID_I2C_RTC_ERROR) {
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->i2cRtcError = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->i2cRtcError = 1;
        }
    }
}
