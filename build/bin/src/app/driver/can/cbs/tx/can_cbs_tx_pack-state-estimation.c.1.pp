...
extern uint32_t CANTX_PackStateEstimation(
    ...
    uint8_t *pMuxId,
    const CAN_SHIM_s *const kpkCanShim) {
    do { if (!(message . id == (0x225u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 81); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(message . idType == (CAN_STANDARD_IDENTIFIER_11_BIT))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 82); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(message . dlc <= (8u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 83); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pCanData != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 84); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pMuxId == ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 85); } while (0); FAS_InfiniteLoop(); } } while (0);  
    do { if (!(kpkCanShim != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 86); } while (0); FAS_InfiniteLoop(); } } while (0);
    uint64_t messageData = 0u;

    float_t minimumStringSoc_perc   = 3.402823466E+38F;
    float_t maximumStringSoc_perc   = 1.175494351E-38F;
    float_t minimumStringSoe_perc   = 3.402823466E+38F;
    float_t maximumStringSoe_perc   = 1.175494351E-38F;
    uint32_t minimumStringEnergy_Wh = 0xffffffffU;

    (DATA_Read1DataBlock) (kpkCanShim->pTableSox);
    ...
    if (BMS_GetNumberOfConnectedStrings() != 0u) {
        if (BMS_GetBatterySystemState() == BMS_CHARGING) {
            packSoc_perc = (BMS_GetNumberOfConnectedStrings() * maximumStringSoc_perc) / (1u);
            packSoe_perc = (BMS_GetNumberOfConnectedStrings() * maximumStringSoe_perc) / (1u);
        } else {
            packSoc_perc = (BMS_GetNumberOfConnectedStrings() * minimumStringSoc_perc) / (1u);
            packSoe_perc = (BMS_GetNumberOfConnectedStrings() * minimumStringSoe_perc) / (1u);
        }
    ...
    float_t offset     = 0.0f;
    float_t factor     = 100.0f;  
    signalData         = (signalData + offset) * factor;
    uint64_t data      = (int64_t)signalData;
     
    CAN_TxSetMessageDataWithSignalData(&messageData, 0u, 14u, data, message.endianness);
    ...
    CAN_TxSetMessageDataWithSignalData(&messageData, 14u, 14u, data, message.endianness);
    ...
    CAN_TxSetMessageDataWithSignalData(&messageData, 40u, 24u, data, message.endianness);
    ...
    CAN_TxSetMessageDataWithSignalData(&messageData, 28u, 12u, data, message.endianness);
...
extern uint32_t ModuleStateEstimation(
    ...
    uint8_t *pMuxId,
    const CAN_SHIM_s *const kpkCanShim) {
    do { if (!(message . id == (0x71Bu))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 197); } while (0); FAS_InfiniteLoop(); } } while (0);
    ...
    do { if (!(pCanData != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 200); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pMuxId == ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 201); } while (0); FAS_InfiniteLoop(); } } while (0);  
    do { if (!(kpkCanShim != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 202); } while (0); FAS_InfiniteLoop(); } } while (0);
    uint64_t messageData = 0u;
