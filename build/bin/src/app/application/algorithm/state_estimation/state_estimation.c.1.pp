...
extern void SE_InitializeSoc(_Bool ccPresent, uint8_t stringNumber) {
    do { if (!(stringNumber < (1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 76); } while (0); FAS_InfiniteLoop(); } } while (0);
    SE_InitializeStateOfCharge(&se_tableSocAndSoeEstimation, ccPresent, stringNumber);
    (DATA_Write1DataBlock) (&se_tableSocAndSoeEstimation);
}

extern void SE_InitializeSoe(_Bool ec_present, uint8_t stringNumber) {
    do { if (!(stringNumber < (1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 82); } while (0); FAS_InfiniteLoop(); } } while (0);
    SE_InitializeStateOfEnergy(&se_tableSocAndSoeEstimation, ec_present, stringNumber);
    (DATA_Write1DataBlock) (&se_tableSocAndSoeEstimation);
}

extern void SE_InitializeSoh(uint8_t stringNumber) {
    do { if (!(stringNumber < (1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 88); } while (0); FAS_InfiniteLoop(); } } while (0);
