...
extern void MATH_StartupSelfTest(void) {
    do { if (!(MATH_AbsInt64_t((-0x7fffffffffffffffLL-1)) == 0x7fffffffffffffffLL)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 80); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(MATH_AbsInt32_t((-0x7fffffff-1)) == 0x7fffffff)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 81); } while (0); FAS_InfiniteLoop(); } } while (0);
}
...
extern float_t MATH_LinearInterpolation(
    ...
    if (fabsf(x1 - x2) >= 1.192092896E-07F) {
...
extern uint32_t MATH_SwapBytesUint32_t(const uint32_t val) {
    ...
    const uint32_t intermediate               = ((val << (8u)) & alternating2PatternStartFF) |
                                  ((val >> (8u)) & alternating2PatternStart00);
    return (intermediate << (16u)) | (intermediate >> (16u));
}
...
extern uint64_t MATH_SwapBytesUint64_t(const uint64_t val) {
    ...
    uint64_t intermediate = ((val << (8u)) & alternating2PatternStartFF) |
                            ((val >> (8u)) & alternating2PatternStart00);
    intermediate = ((intermediate << (16u)) & alternating4PatternStartFFFF) |
                   ((intermediate >> (16u)) & alternating4PatternStart0000);
    return (intermediate << (32u)) | (intermediate >> (32u));
}
...
extern int32_t MATH_AbsInt32_t(const int32_t value) {
    int32_t absoluteValue = 0x7fffffff;
...
extern int64_t MATH_AbsInt64_t(const int64_t value) {
    int64_t absoluteValue = 0x7fffffffffffffffLL;
