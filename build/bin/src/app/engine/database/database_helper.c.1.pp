...
extern _Bool DATA_EntryUpdatedWithinInterval(DATA_BLOCK_HEADER_s dataBlockHeader, uint32_t timeInterval) {
    ...
    const _Bool updatedAtLeastOnce         = DATA_DatabaseEntryUpdatedAtLeastOnce(dataBlockHeader);
    if ((timeDifferenceLastCall <= timeInterval) && (updatedAtLeastOnce == 1)) {
...
extern _Bool DATA_EntryUpdatedPeriodicallyWithinInterval(DATA_BLOCK_HEADER_s dataBlockHeader, uint32_t timeInterval) {
    ...
    if ((timeDifferenceLastCall <= timeInterval) && (timeDifferenceBetweenCalls <= timeInterval) &&
        (DATA_DatabaseEntryUpdatedAtLeastOnce(dataBlockHeader) == 1)) {
...
extern uint8_t DATA_GetStringNumberFromVoltageIndex(uint16_t cellIndex) {
    do { if (!(cellIndex < (((3u) * (16u)) * (1u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 121); } while (0); FAS_InfiniteLoop(); } } while (0);
    return (uint8_t)(cellIndex / ((3u) * (16u)));
}

extern uint8_t DATA_GetModuleNumberFromVoltageIndex(uint16_t cellIndex) {
    do { if (!(cellIndex < (((3u) * (16u)) * (1u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 126); } while (0); FAS_InfiniteLoop(); } } while (0);
    uint8_t stringNumber = DATA_GetStringNumberFromVoltageIndex(cellIndex);
    uint8_t moduleNumber =
        (uint8_t)((cellIndex / (16u)) - (stringNumber * (3u)));
    return moduleNumber;
}

extern uint8_t DATA_GetCellNumberFromVoltageIndex(uint16_t cellIndex) {
    do { if (!(cellIndex < (((3u) * (16u)) * (1u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 134); } while (0); FAS_InfiniteLoop(); } } while (0);
    return (uint8_t)(cellIndex % (16u));
}

extern uint8_t DATA_GetStringNumberFromTemperatureIndex(uint16_t sensorIndex) {
    do { if (!(sensorIndex < (((3u) * (8u)) * (1u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 139); } while (0); FAS_InfiniteLoop(); } } while (0);
    return (uint8_t)(sensorIndex / ((3u) * (8u)));
}

extern uint8_t DATA_GetModuleNumberFromTemperatureIndex(uint16_t sensorIndex) {
    do { if (!(sensorIndex < (((3u) * (8u)) * (1u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 144); } while (0); FAS_InfiniteLoop(); } } while (0);
    uint8_t stringNumber = DATA_GetStringNumberFromTemperatureIndex(sensorIndex);
    uint8_t moduleNumber =
        (uint8_t)((sensorIndex / (8u)) - (stringNumber * (3u)));
    return moduleNumber;
}

extern uint8_t DATA_GetSensorNumberFromTemperatureIndex(uint16_t sensorIndex) {
    do { if (!(sensorIndex < (((3u) * (8u)) * (1u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 152); } while (0); FAS_InfiniteLoop(); } } while (0);
    return (uint8_t)(sensorIndex % (8u));
}
