...
extern void DIAG_Sbc(
    ...
    if (diagId == DIAG_ID_SBC_FIN_STATE) {
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->mcuSbcFinError = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->mcuSbcFinError = 1;
        }
    } else if (diagId == DIAG_ID_SBC_RSTB_STATE) {
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->mcuSbcRstbError = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->mcuSbcRstbError = 1;
        }
    }
}
