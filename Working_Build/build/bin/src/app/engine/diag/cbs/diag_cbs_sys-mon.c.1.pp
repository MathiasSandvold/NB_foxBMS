...
static _Bool DIAG_EventToBool(DIAG_EVENT_e event);
...
extern void DIAG_ErrorSystemMonitoring(
    ...
    const DIAG_DATABASE_SHIM_s *const kpkDiagShim,
    uint32_t data) {
    do { if (!(diagId == DIAG_ID_SYSTEM_MONITORING)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 93); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!((event == DIAG_EVENT_OK) || (event == DIAG_EVENT_NOT_OK) || (event == DIAG_EVENT_RESET))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 94); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(kpkDiagShim != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 95); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(data < (uint32_t)SYSM_TASK_ID_MAX)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 96); } while (0); FAS_InfiniteLoop(); } } while (0);
    ...
    if (data == (uint32_t)SYSM_TASK_ID_ENGINE) {
        ...
        do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 109); } while (0); FAS_InfiniteLoop(); } } while (0);
    }
}
