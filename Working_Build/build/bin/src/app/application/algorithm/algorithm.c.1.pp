...
static _Bool algo_initializationRequested = 0;
...
static void ALGO_Initialization(void) {
    ...
    for (uint16_t i = 0u; i < algo_length; i++) {
        ...
        do { if (!((algo_algorithms[i]. cycleTime_ms % (100u)) == 0u)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 88); } while (0); FAS_InfiniteLoop(); } } while (0);

         
        if (algo_algorithms[i].state == ALGO_UNINITIALIZED) {
             
            if (algo_algorithms[i].fpInitialization == ((void *)(0u))) {
                algo_algorithms[i].state = ALGO_READY;
            } else {
                const STD_RETURN_TYPE_e result = algo_algorithms[i].fpInitialization();
                do { if (!((STD_OK == result) || (STD_NOT_OK == result))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 97); } while (0); FAS_InfiniteLoop(); } } while (0);
                if (STD_OK == result) {
                    algo_algorithms[i].state = ALGO_READY;
                } else {
...
extern void ALGO_UnlockInitialization(void) {
    ...
    algo_initializationRequested = 1;
    OS_ExitTaskCritical();
}

extern void ALGO_MainFunction(void) {
    OS_EnterTaskCritical();
    const _Bool initializationRequested = algo_initializationRequested;
    OS_ExitTaskCritical();
    if (initializationRequested == 1) {
        ALGO_Initialization();
        OS_EnterTaskCritical();
        algo_initializationRequested = 0;
        OS_ExitTaskCritical();
    }
    ...
    for (uint16_t i = 0u; i < algo_length; i++) {
        const _Bool runAlgorithmAsap = (algo_algorithms[i].cycleTime_ms == 0u);
        const _Bool runAlgorithmCycleElapsed =
            ((algo_algorithms[i].cycleTime_ms != 0u) && ((counter_ticks % algo_algorithms[i].cycleTime_ms) == 0u));
        if ((runAlgorithmAsap != 0) || (runAlgorithmCycleElapsed != 0)) {
