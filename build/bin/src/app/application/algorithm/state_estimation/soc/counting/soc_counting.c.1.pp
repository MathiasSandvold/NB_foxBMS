...
typedef struct {
    _Bool socInitialized;                  
    _Bool sensorCcUsed[(1u)];  
    float_t ccScalingAverage[(1u)];    
    float_t ccScalingMinimum[(1u)];    
    float_t ccScalingMaximum[(1u)];    
    uint32_t previousTimestamp[(1u)];  
} SOC_STATE_s;
...
static SOC_STATE_s soc_state = {
    .socInitialized    = 0,
    .sensorCcUsed      = {(0)},
    .ccScalingAverage  = {(0.0f)},
    .ccScalingMinimum  = {(0.0f)},
    .ccScalingMaximum  = {(0.0f)},
...
static float_t SOC_GetStringSocPercentageFromCharge(uint32_t charge_As) {
    const float_t charge_mAs = (float_t)charge_As * (1000.0f);
    return (100.0f) * (charge_mAs / ((float_t)(((float_t)((1u) * (13000u))) * 3600.0f)));
}

static void SOC_RecalibrateViaLookupTable(DATA_BLOCK_SOX_s *pTableSoc) {
    do { if (!(pTableSoc != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 159); } while (0); FAS_InfiniteLoop(); } } while (0);
    DATA_BLOCK_MIN_MAX_s tableMinMaxCellVoltages = {.header.uniqueId = DATA_BLOCK_ID_MIN_MAX};
    (DATA_Read1DataBlock) (&tableMinMaxCellVoltages);
...
static void SOC_SetValue(
    ...
    float_t socAverageValue_perc,
    uint8_t stringNumber) {
    do { if (!(pTableSoc != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 180); } while (0); FAS_InfiniteLoop(); } } while (0);
    ...
    if (soc_state.sensorCcUsed[stringNumber] == 1) {
...
static void SOC_CheckDatabaseSocPercentageLimits(DATA_BLOCK_SOX_s *pTableSoc, uint8_t stringNumber) {
    do { if (!(pTableSoc != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 211); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(stringNumber < (1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 212); } while (0); FAS_InfiniteLoop(); } } while (0);
...
static void SOC_UpdateNvmValues(DATA_BLOCK_SOX_s *pTableSoc, uint8_t stringNumber) {
    do { if (!(pTableSoc != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 235); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(stringNumber < (1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 236); } while (0); FAS_InfiniteLoop(); } } while (0);
...
void SE_InitializeStateOfCharge(DATA_BLOCK_SOX_s *pSocValues, _Bool ccPresent, uint8_t stringNumber) {
    do { if (!(pSocValues != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 245); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(stringNumber < (1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 246); } while (0); FAS_InfiniteLoop(); } } while (0);
    (DATA_Read1DataBlock) (&soc_tableCurrentSensor);

    FRAM_ReadData(FRAM_BLOCK_ID_SOC);

    if (ccPresent == 1) {
        soc_state.sensorCcUsed[stringNumber] = 1;
        ...
        soc_state.sensorCcUsed[stringNumber]      = 0;
    }
    ...
    soc_state.socInitialized = 1;
}

 
void SE_CalculateStateOfCharge(DATA_BLOCK_SOX_s *pSocValues) {
     
    do { if (!(pSocValues != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 288); } while (0); FAS_InfiniteLoop(); } } while (0);
    _Bool continueFunction = 1;
    if (soc_state.socInitialized == 0) {
                ...
                if (soc_state.sensorCcUsed[s] == 0) {
                    ...
                    if (soc_state.previousTimestamp[s] != soc_tableCurrentSensor.timestampCurrent[s]) {
                        ...
                        if (timeStep_s > 0.0f) {
                            ...
                            float_t deltaSOC_perc = (((float_t)soc_tableCurrentSensor.current_mA[s] * timeStep_s) /
                                                     ((float_t)(((float_t)((1u) * (13000u))) * 3600.0f))) *
                                                    100.0f / 1000.0f;  
                    ...
                    if (soc_state.previousTimestamp[s] != soc_tableCurrentSensor.timestampCurrentCounting[s]) {
                        float_t deltaSoc_perc =
                            ((float_t)soc_tableCurrentSensor.currentCounter_As[s] / ((float_t)(((float_t)(((float_t)((1u) * (13000u))) * 3600.0f)) / 1000.0f))) * 100.0f;
...
extern float_t SE_GetStateOfChargeFromVoltage(int16_t voltage_mV) {
    ...
    if (!(((between_high == 0u) && (between_low == 0u)) ||        
          (between_low >= bc_stateOfChargeLookupTableLength))) {  
