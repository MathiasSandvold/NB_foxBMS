...
extern void DIAG_ErrorInterlock(
    ...
    const DIAG_DATABASE_SHIM_s *const kpkDiagShim,
    uint32_t data) {
    do { if (!(diagId == DIAG_ID_INTERLOCK_FEEDBACK)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 78); } while (0); FAS_InfiniteLoop(); } } while (0);
    ...
    if (event == DIAG_EVENT_RESET) {
        kpkDiagShim->pTableError->interlockOpenedError = 0;
    }
    if (event == DIAG_EVENT_NOT_OK) {
        kpkDiagShim->pTableError->interlockOpenedError = 1;
    }
}
