...
static _Bool CANRX_CheckIfBmsSoftwareVersionIsRequested(uint64_t messageData, CAN_ENDIANNESS_e endianness);
...
static _Bool CANRX_CheckIfMcuUniqueDieIdIsRequested(uint64_t messageData, CAN_ENDIANNESS_e endianness);
...
static _Bool CANRX_CheckIfMcuLotNumberIsRequested(uint64_t messageData, CAN_ENDIANNESS_e endianness);
...
static _Bool CANRX_CheckIfMcuWaferInformationIsRequested(uint64_t messageData, CAN_ENDIANNESS_e endianness);
...
static _Bool CANRX_CheckIfSoftwareResetIsRequested(uint64_t messageData, CAN_ENDIANNESS_e endianness);
...
static _Bool CANRX_CheckIfFramInitializationIsRequested(uint64_t messageData, CAN_ENDIANNESS_e endianness);
...
static _Bool CANRX_CheckIfTimeInfoIsRequested(uint64_t messageData, CAN_ENDIANNESS_e endianness);
...
static uint8_t CANRX_GetHundredthOfSeconds(uint64_t messageData, CAN_ENDIANNESS_e endianness) {
    ...
    do { if (!(endianness == CAN_BIG_ENDIAN)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 342); } while (0); FAS_InfiniteLoop(); } } while (0);
...
static uint8_t CANRX_GetSeconds(uint64_t messageData, CAN_ENDIANNESS_e endianness) {
    ...
    do { if (!(endianness == CAN_BIG_ENDIAN)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 358); } while (0); FAS_InfiniteLoop(); } } while (0);
...
static uint8_t CANRX_GetMinutes(uint64_t messageData, CAN_ENDIANNESS_e endianness) {
    ...
    do { if (!(endianness == CAN_BIG_ENDIAN)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 375); } while (0); FAS_InfiniteLoop(); } } while (0);
...
static uint8_t CANRX_GetHours(uint64_t messageData, CAN_ENDIANNESS_e endianness) {
    ...
    do { if (!(endianness == CAN_BIG_ENDIAN)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 392); } while (0); FAS_InfiniteLoop(); } } while (0);

    uint64_t signalData = 0u;
     
    CAN_RxGetSignalDataFromMessageData(
        messageData, (28u), (5u), &signalData, endianness);
...
static uint8_t CANRX_GetWeekday(uint64_t messageData, CAN_ENDIANNESS_e endianness) {
    ...
    do { if (!(endianness == CAN_BIG_ENDIAN)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 405); } while (0); FAS_InfiniteLoop(); } } while (0);
...
static uint8_t CANRX_GetDay(uint64_t messageData, CAN_ENDIANNESS_e endianness) {
    ...
    do { if (!(endianness == CAN_BIG_ENDIAN)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 422); } while (0); FAS_InfiniteLoop(); } } while (0);

    uint64_t signalData = 0u;
     
    CAN_RxGetSignalDataFromMessageData(
        messageData, (36u), (5u), &signalData, endianness);
...
static uint8_t CANRX_GetMonth(uint64_t messageData, CAN_ENDIANNESS_e endianness) {
    ...
    do { if (!(endianness == CAN_BIG_ENDIAN)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 435); } while (0); FAS_InfiniteLoop(); } } while (0);

     
    uint64_t signalData = 0u;
    CAN_RxGetSignalDataFromMessageData(
        messageData, (47u), (4u), &signalData, endianness);
...
static uint8_t CANRX_GetYear(uint64_t messageData, CAN_ENDIANNESS_e endianness) {
    ...
    do { if (!(endianness == CAN_BIG_ENDIAN)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 448); } while (0); FAS_InfiniteLoop(); } } while (0);

     
    uint64_t signalData = 0u;
    CAN_RxGetSignalDataFromMessageData(
        messageData, (43u), (7u), &signalData, endianness);
...
static void CANRX_ProcessRtcMux(uint64_t messageData, CAN_ENDIANNESS_e endianness) {
    ...
    do { if (!(endianness == CAN_BIG_ENDIAN)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 461); } while (0); FAS_InfiniteLoop(); } } while (0);
...
static _Bool CANRX_CheckIfBmsSoftwareVersionIsRequested(uint64_t messageData, CAN_ENDIANNESS_e endianness) {
    ...
    do { if (!(endianness == CAN_BIG_ENDIAN)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 480); } while (0); FAS_InfiniteLoop(); } } while (0);
...
static void CANRX_TriggerBmsSoftwareVersionMessage(void) {
    ...
    if (CANTX_DebugResponse(CANTX_DEBUG_RESPONSE_TRANSMIT_BMS_VERSION_INFO) != STD_OK) {
        do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 501); } while (0); FAS_InfiniteLoop(); } } while (0);
    }
}

static _Bool CANRX_CheckIfMcuUniqueDieIdIsRequested(uint64_t messageData, CAN_ENDIANNESS_e endianness) {
     
    do { if (!(endianness == CAN_BIG_ENDIAN)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 507); } while (0); FAS_InfiniteLoop(); } } while (0);
...
static void CANRX_TriggerMcuUniqueDieIdMessage(void) {
    ...
    if (CANTX_DebugResponse(CANTX_DEBUG_RESPONSE_TRANSMIT_MCU_UNIQUE_DIE_ID) != STD_OK) {
        do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 528); } while (0); FAS_InfiniteLoop(); } } while (0);
    }
}

static _Bool CANRX_CheckIfMcuLotNumberIsRequested(uint64_t messageData, CAN_ENDIANNESS_e endianness) {
     
    do { if (!(endianness == CAN_BIG_ENDIAN)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 534); } while (0); FAS_InfiniteLoop(); } } while (0);
...
static void CANRX_TriggerMcuLotNumberMessage(void) {
    ...
    if (CANTX_DebugResponse(CANTX_DEBUG_RESPONSE_TRANSMIT_MCU_LOT_NUMBER) != STD_OK) {
        do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 555); } while (0); FAS_InfiniteLoop(); } } while (0);
    }
}

static _Bool CANRX_CheckIfMcuWaferInformationIsRequested(uint64_t messageData, CAN_ENDIANNESS_e endianness) {
     
    do { if (!(endianness == CAN_BIG_ENDIAN)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 561); } while (0); FAS_InfiniteLoop(); } } while (0);
...
static void CANRX_TriggerMcuWaferInformationMessage(void) {
    ...
    if (CANTX_DebugResponse(CANTX_DEBUG_RESPONSE_TRANSMIT_MCU_WAFER_INFORMATION) != STD_OK) {
        do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 582); } while (0); FAS_InfiniteLoop(); } } while (0);
    }
}

static void CANRX_ProcessVersionInformationMux(uint64_t messageData, CAN_ENDIANNESS_e endianness) {
     
    do { if (!(endianness == CAN_BIG_ENDIAN)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 588); } while (0); FAS_InfiniteLoop(); } } while (0);

     
    if (CANRX_CheckIfBmsSoftwareVersionIsRequested(messageData, endianness) == 1) {
        CANRX_TriggerBmsSoftwareVersionMessage();
    }
    if (CANRX_CheckIfMcuUniqueDieIdIsRequested(messageData, endianness) == 1) {
        CANRX_TriggerMcuUniqueDieIdMessage();
    }
    if (CANRX_CheckIfMcuLotNumberIsRequested(messageData, endianness) == 1) {
        CANRX_TriggerMcuLotNumberMessage();
    }
    if (CANRX_CheckIfMcuWaferInformationIsRequested(messageData, endianness) == 1) {
...
static _Bool CANRX_CheckIfSoftwareResetIsRequested(uint64_t messageData, CAN_ENDIANNESS_e endianness) {
    ...
    do { if (!(endianness == CAN_BIG_ENDIAN)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 607); } while (0); FAS_InfiniteLoop(); } } while (0);
...
static _Bool CANRX_CheckIfFramInitializationIsRequested(uint64_t messageData, CAN_ENDIANNESS_e endianness) {
    ...
    do { if (!(endianness == CAN_BIG_ENDIAN)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 627); } while (0); FAS_InfiniteLoop(); } } while (0);
...
static void CANRX_ProcessSoftwareResetMux(uint64_t messageData, CAN_ENDIANNESS_e endianness) {
    ...
    do { if (!(endianness == CAN_BIG_ENDIAN)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 647); } while (0); FAS_InfiniteLoop(); } } while (0);

     
    if (CANRX_CheckIfSoftwareResetIsRequested(messageData, endianness) == 1) {
...
static void CANRX_ProcessFramInitializationMux(uint64_t messageData, CAN_ENDIANNESS_e endianness) {
    ...
    do { if (!(endianness == CAN_BIG_ENDIAN)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 657); } while (0); FAS_InfiniteLoop(); } } while (0);

     
    if (CANRX_CheckIfFramInitializationIsRequested(messageData, endianness) == 1) {
...
static void CANRX_ProcessTimeInfoMux(uint64_t messageData, CAN_ENDIANNESS_e endianness) {
    ...
    do { if (!(endianness == CAN_BIG_ENDIAN)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 667); } while (0); FAS_InfiniteLoop(); } } while (0);

     
    if (CANRX_CheckIfTimeInfoIsRequested(messageData, endianness) == 1) {
...
static _Bool CANRX_CheckIfTimeInfoIsRequested(uint64_t messageData, CAN_ENDIANNESS_e endianness) {
    ...
    do { if (!(endianness == CAN_BIG_ENDIAN)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 677); } while (0); FAS_InfiniteLoop(); } } while (0);
...
static void CANRX_TriggerTimeInfoMessage(void) {
    ...
    if (CANTX_DebugResponse(CANTX_DEBUG_RESPONSE_TRANSMIT_RTC_TIME) != STD_OK) {
        do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 698); } while (0); FAS_InfiniteLoop(); } } while (0);
    }
}
...
extern uint32_t CANRX_Debug(
    ...
    const uint8_t *const kpkCanData,
    const CAN_SHIM_s *const kpkCanShim) {
    do { if (!(message . id == (0x200u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 707); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(message . idType == (CAN_STANDARD_IDENTIFIER_11_BIT))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 708); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(message . dlc == (8u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 709); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(kpkCanData != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 710); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(kpkCanShim != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 711); } while (0); FAS_InfiniteLoop(); } } while (0);
    ...
    CAN_RxGetSignalDataFromMessageData(
        messageData, (0x7u), (8u), &muxValue, message.endianness);
