...
_Bool SOA_IsPackCurrentLimitViolated(uint32_t current_mA, BMS_CURRENT_FLOW_STATE_e currentDirection) {
    _Bool currentLimitViolated = 0;
     
    if ((currentDirection == BMS_CHARGING) || (currentDirection == BMS_DISCHARGING)) {
        if (current_mA > (780000u * (1u))) {
            currentLimitViolated = 1;
        }
    }
    return currentLimitViolated;
}

_Bool SOA_IsStringCurrentLimitViolated(uint32_t current_mA, BMS_CURRENT_FLOW_STATE_e currentDirection) {
    _Bool currentLimitViolated = 0;
     
    if ((currentDirection == BMS_CHARGING) || (currentDirection == BMS_DISCHARGING)) {
        if (current_mA > (780000u)) {
            currentLimitViolated = 1;
        }
    }
    return currentLimitViolated;
...
_Bool SOA_IsCellCurrentLimitViolated(uint32_t current_mA, BMS_CURRENT_FLOW_STATE_e currentDirection) {
    _Bool currentLimitViolated = 0;
     
    if (currentDirection == BMS_CHARGING) {
        if (current_mA > ((1u) * (780000u))) {
            currentLimitViolated = 1;
        }
        ...
        currentLimitViolated = 0;
    }
    return currentLimitViolated;
}

_Bool SOA_IsCurrentOnOpenString(BMS_CURRENT_FLOW_STATE_e currentDirection, uint8_t stringNumber) {
    do { if (!(stringNumber < (1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 116); } while (0); FAS_InfiniteLoop(); } } while (0);
    _Bool currentOnOpenString = 0;
     
    if ((currentDirection == BMS_CHARGING) || (currentDirection == BMS_DISCHARGING)) {
        const _Bool stringClosed      = BMS_IsStringClosed(stringNumber);
        const _Bool stringPrecharging = BMS_IsStringPrecharging(stringNumber);
         
        if ((stringClosed == 0) && (stringPrecharging == 0)) {
            currentOnOpenString = 1;
        }
    }
