...
static IMD_FSM_STATES_e IR155_InitializeModule(void) {
    ...
    IR155_Initialize(((100u)));
    return IMD_FSM_STATE_IMD_ENABLE;
}

static IMD_FSM_STATES_e IR155_MeasureInsulation(DATA_BLOCK_INSULATION_MONITORING_s *pTableInsulationMonitoring) {
    do { if (!(pTableInsulationMonitoring != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 114); } while (0); FAS_InfiniteLoop(); } } while (0);
    pTableInsulationMonitoring->isInsulationMeasurementValid   = 1;
    pTableInsulationMonitoring->insulationResistance_kOhm      = (0u);
    ...
    if (ir155_state.timeUntilValidMeasurement_ms > 0u) {
        ...
        if (ir155_state.timeUntilValidMeasurement_ms == 0u) {
            ...
            fram_insulationFlags.groundErrorDetected = 0;
        }
    }
    ...
    if (pTableInsulationMonitoring->insulationResistance_kOhm <= (750u)) {
...
extern IMD_FSM_STATES_e IMD_ProcessEnableState(void) {
    ...
    IO_PinSet(&(((hetBASE_t *)0xFFF7B800U))->DOUT, (25u));
    return IMD_FSM_STATE_RUNNING;
}

extern IMD_FSM_STATES_e IMD_ProcessRunningState(DATA_BLOCK_INSULATION_MONITORING_s *pTableInsulationMonitoring) {
    do { if (!(pTableInsulationMonitoring != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 221); } while (0); FAS_InfiniteLoop(); } } while (0);
    return IR155_MeasureInsulation(pTableInsulationMonitoring);
}

extern IMD_FSM_STATES_e IMD_ProcessShutdownState(void) {
     
    IO_PinReset(&(((hetBASE_t *)0xFFF7B800U))->DOUT, (25u));
    return IMD_FSM_STATE_IMD_ENABLE;
}
