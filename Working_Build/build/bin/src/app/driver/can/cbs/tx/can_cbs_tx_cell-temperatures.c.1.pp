...
static const CAN_SIGNAL_TYPE_s cantx_cellTemperatureMultiplexer  = {0u, 8u, 1.0f, 0.0f, 0.0f, 1.0f};
...
static const CAN_SIGNAL_TYPE_s cantx_cell0Temperature_degC       = {16u, 8u, 1.0f, 0.0f, -128.0f, 127.0f};
static const CAN_SIGNAL_TYPE_s cantx_cell1Temperature_degC       = {24u, 8u, 1.0f, 0.0f, -128.0f, 127.0f};
static const CAN_SIGNAL_TYPE_s cantx_cell2Temperature_degC       = {32u, 8u, 1.0f, 0.0f, -128.0f, 127.0f};
static const CAN_SIGNAL_TYPE_s cantx_cell3Temperature_degC       = {40u, 8u, 1.0f, 0.0f, -128.0f, 127.0f};
static const CAN_SIGNAL_TYPE_s cantx_cell4Temperature_degC       = {48u, 8u, 1.0f, 0.0f, -128.0f, 127.0f};
static const CAN_SIGNAL_TYPE_s cantx_cell5Temperature_degC       = {56u, 8u, 1.0f, 0.0f, -128.0f, 127.0f};
    ...
    if (muxId < (((3u) * (8u)) * (1u))) {
        ...
        float_t signalData_degC =
            (float_t)kpkCanShim->pTableCellTemperature
                ->cellTemperature_ddegC[stringNumber][(moduleNumber * (8u)) + sensorNumber];
        signalData_degC /= (10.0f);  
...
extern uint32_t CANTX_CellTemperatures(
    ...
    uint8_t *pMuxId,
    const CAN_SHIM_s *const kpkCanShim) {
    do { if (!(message . id == (0x250u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 178); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(message . idType == (CAN_STANDARD_IDENTIFIER_11_BIT))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 179); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(message . dlc == (8u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 180); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pCanData != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 181); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pMuxId != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 182); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(kpkCanShim != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 183); } while (0); FAS_InfiniteLoop(); } } while (0);
    ...
    if (*pMuxId == 0u) {
        (DATA_Read1DataBlock) (kpkCanShim->pTableCellTemperature);
    }

     
    uint32_t signalData = *pMuxId / 6u;  
    CAN_TxSetMessageDataWithSignalData(
        &messageData,
