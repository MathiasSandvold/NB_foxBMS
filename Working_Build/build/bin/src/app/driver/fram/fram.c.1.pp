...
extern void FRAM_Initialize(void) {
    ...
    for (uint16_t i = 0u; i < FRAM_BLOCK_MAX; i++) {
        ...
        address += (fram_base_header[i]).datalength + (sizeof(uint64_t));
    }

     
    do { if (!(!(address > (0x3FFFFu)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 108); } while (0); FAS_InfiniteLoop(); } } while (0);
...
extern STD_RETURN_TYPE_e FRAM_ReinitializeAllEntries(void) {
    ...
    fram_version.project = ((FRAM_PROJECT_ID)0u);
...
extern FRAM_RETURN_TYPE_e FRAM_WriteData(FRAM_BLOCK_ID_e blockId) {
    do { if (!(blockId < FRAM_BLOCK_MAX)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 134); } while (0); FAS_InfiniteLoop(); } } while (0);
    ...
    do { if (!(spi_framInterface . csType == SPI_CHIP_SELECT_SOFTWARE)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 141); } while (0); FAS_InfiniteLoop(); } } while (0);
...
extern FRAM_RETURN_TYPE_e FRAM_ReadData(FRAM_BLOCK_ID_e blockId) {
    do { if (!(blockId < FRAM_BLOCK_MAX)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 212); } while (0); FAS_InfiniteLoop(); } } while (0);

    uint16_t read             = 0u;
    FRAM_RETURN_TYPE_e retVal = FRAM_ACCESS_OK;

     
    do { if (!(spi_framInterface . csType == SPI_CHIP_SELECT_SOFTWARE)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 218); } while (0); FAS_InfiniteLoop(); } } while (0);
