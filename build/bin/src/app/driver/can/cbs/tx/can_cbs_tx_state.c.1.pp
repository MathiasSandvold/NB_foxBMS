...
typedef struct {
    _Bool recordedViolationAny;        
    _Bool recordedViolationEngine;     
    _Bool recordedViolation1ms;        
    _Bool recordedViolation10ms;       
    _Bool recordedViolation100ms;      
    _Bool recordedViolation100msAlgo;  
...
static _Bool CANTX_AnySysMonTimingIssueDetected(const CAN_SHIM_s *const kpkCanShim);

 
static _Bool CANTX_AnySysMonTimingIssueDetected(const CAN_SHIM_s *const kpkCanShim) {
    do { if (!(kpkCanShim != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 78); } while (0); FAS_InfiniteLoop(); } } while (0);
    SYSM_TIMING_VIOLATION_RESPONSE_s recordedTimingViolations = {0};
    SYSM_GetRecordedTimingViolations(&recordedTimingViolations);

    const _Bool anyTimingViolation =
...
extern uint32_t CANTX_BmsState(
    ...
    uint8_t *pMuxId,
    const CAN_SHIM_s *const kpkCanShim) {
    do { if (!(message . id == (0x359u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 99); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(message . dlc == (8u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 100); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pCanData != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 101); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pMuxId == ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 102); } while (0); FAS_InfiniteLoop(); } } while (0);  
    do { if (!(kpkCanShim != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 103); } while (0); FAS_InfiniteLoop(); } } while (0);
    uint64_t messageData = 0u;

    (DATA_Read3DataBlocks) (kpkCanShim->pTableErrorState, kpkCanShim->pTableInsulation, kpkCanShim->pTableMsl);

     
     
    uint64_t data = (uint64_t)BMS_GetState();
     
    CAN_TxSetMessageDataWithSignalData(&messageData, 0u, 4u, data, message.endianness);

     
    data = (uint64_t)BMS_GetNumberOfConnectedStrings();

     
    CAN_TxSetMessageDataWithSignalData(&messageData, 4u, 4u, data, message.endianness);
    ...
    for (uint8_t s = 0u; s < (1u); s++) {
        if (kpkCanShim->pTableErrorState->prechargeAbortedDueToVoltage[s] == 1) {
            data = 1u;
        }
    }
    ...
    data = 0u;  
    for (uint8_t s = 0u; s < (1u); s++) {
        if (kpkCanShim->pTableErrorState->prechargeAbortedDueToCurrent[s] == 1) {
            data = 1u;
        }
    }
    ...
    for (uint8_t i = 0; i < 11u; i++) {
        data = DIAG_GetDiagnosisEntryState(persistant_error_flags[i].id);

        if (data) {
            persistant_error_flags[i].latched_value = data;
        }

        CAN_TxSetMessageDataWithSignalData(
            &messageData,
            persistant_error_flags[i].can_offset,
            1u,
            persistant_error_flags[i].latched_value,
            message.endianness);
    }
...
extern uint32_t CANTX_BmsStateDetails(
    ...
    uint8_t *pMuxId,
    const CAN_SHIM_s *const kpkCanShim) {
    do { if (!(message . id == (0x226u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 235); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(message . idType == (CAN_STANDARD_IDENTIFIER_11_BIT))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 236); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(message . dlc == (8u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 237); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pCanData != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 238); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pMuxId == ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 239); } while (0); FAS_InfiniteLoop(); } } while (0);  
    do { if (!(kpkCanShim != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 240); } while (0); FAS_InfiniteLoop(); } } while (0);
    uint64_t messageData = 0u;

    (DATA_Read1DataBlock) (kpkCanShim->pTableErrorState);
    SYSM_TIMING_VIOLATION_RESPONSE_s recordedTimingViolations = {0};
    SYSM_GetRecordedTimingViolations(&recordedTimingViolations);
...
extern uint32_t CANTX_StringState(
    ...
    uint8_t *pMuxId,
    const CAN_SHIM_s *const kpkCanShim) {
    do { if (!(message . id == (0x71Eu))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 292); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(message . idType == (CAN_STANDARD_IDENTIFIER_11_BIT))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 293); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(message . dlc == (8u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 294); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pCanData != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 295); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pMuxId != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 296); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(*pMuxId < (1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 297); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(kpkCanShim != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 298); } while (0); FAS_InfiniteLoop(); } } while (0);
    uint64_t messageData = 0u;
    uint64_t data        = 0;
    ...
    if (stringNumber == 0u) {
        ...
        (DATA_Read2DataBlocks) (kpkCanShim->pTableRsl, kpkCanShim->pTableMol);
    }
    ...
    if ((kpkCanShim->pTableErrorState->currentMeasurementInvalidError[stringNumber] == 1) ||
        (kpkCanShim->pTableErrorState->currentMeasurementTimeoutError[stringNumber] == 1)) {
        data = 1u;
    } else {
        data = 0u;
    }
