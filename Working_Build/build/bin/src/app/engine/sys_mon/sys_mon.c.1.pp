...
static volatile _Bool sysm_flagFramCopyHasChanges = 0;
...
static _Bool SYSM_ConvertRecordedTimingsToViolation(uint32_t duration, uint32_t timestampEnter);

 
static void SYSM_RecordTimingViolation(SYSM_TASK_ID_e taskId, uint32_t taskDuration, uint32_t timestampEnter) {
    do { if (!(taskId < SYSM_TASK_ID_MAX)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 98); } while (0); FAS_InfiniteLoop(); } } while (0);

    sysm_flagFramCopyHasChanges               = 1;
    sysm_localFramCopy.anyTimingIssueOccurred = 1;
    ...
    switch (taskId) {
        ...
        default:
            ...
            do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 126); } while (0); FAS_InfiniteLoop(); } } while (0);
            break;
    }
}
...
void SYSM_Notify(SYSM_TASK_ID_e taskId, SYSM_NOTIFY_TYPE_e state, uint32_t timestamp) {
    do { if (!(taskId < SYSM_TASK_ID_MAX)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 186); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!((state == SYSM_NOTIFY_ENTER) || (state == SYSM_NOTIFY_EXIT))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 187); } while (0); FAS_InfiniteLoop(); } } while (0);
    ...
    if (state == SYSM_NOTIFY_ENTER) {
        ...
        do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 197); } while (0); FAS_InfiniteLoop(); } } while (0);
    }
    OS_ExitTaskCritical();
}

extern void SYSM_GetRecordedTimingViolations(SYSM_TIMING_VIOLATION_RESPONSE_s *pAnswer) {
    do { if (!(pAnswer != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 203); } while (0); FAS_InfiniteLoop(); } } while (0);
    ...
    if (localSysMonRecord.anyTimingIssueOccurred == 0) {
         
        pAnswer->recordedViolationAny       = 0;
        pAnswer->recordedViolationEngine    = 0;
        pAnswer->recordedViolation1ms       = 0;
        pAnswer->recordedViolation10ms      = 0;
        pAnswer->recordedViolation100ms     = 0;
        pAnswer->recordedViolation100msAlgo = 0;
    } else {
         
        pAnswer->recordedViolationAny    = 1;
...
extern void SYSM_ClearAllTimingViolations(void) {
    ...
    OS_EnterTaskCritical();
    sysm_localFramCopy.anyTimingIssueOccurred              = 0;
...
extern void SYSM_UpdateFramData(void) {
    OS_EnterTaskCritical();
    const _Bool updateNecessary = sysm_flagFramCopyHasChanges;
    OS_ExitTaskCritical();
    if (updateNecessary == 1) {
        OS_EnterTaskCritical();
        SYSM_CopyFramStruct(&sysm_localFramCopy, &fram_sys_mon_record);
        sysm_flagFramCopyHasChanges = 0;
        OS_ExitTaskCritical();
...
extern void SYSM_CopyFramStruct(const FRAM_SYS_MON_RECORD_s *const kpkFrom, FRAM_SYS_MON_RECORD_s *pTo) {
    do { if (!(kpkFrom != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 273); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pTo != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 274); } while (0); FAS_InfiniteLoop(); } } while (0);
