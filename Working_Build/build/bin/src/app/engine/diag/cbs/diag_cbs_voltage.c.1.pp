...
extern void DIAG_ErrorUndervoltage(
    ...
    const DIAG_DATABASE_SHIM_s *const kpkDiagShim,
    uint32_t stringNumber) {
    do { if (!(diagId < DIAG_ID_MAX)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 111); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!((event == DIAG_EVENT_OK) || (event == DIAG_EVENT_NOT_OK) || (event == DIAG_EVENT_RESET))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 112); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(kpkDiagShim != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 113); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(stringNumber < (1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 114); } while (0); FAS_InfiniteLoop(); } } while (0);
...
extern void DIAG_ErrorHighVoltageMeasurement(
    ...
    const DIAG_DATABASE_SHIM_s *const kpkDiagShim,
    uint32_t stringNumber) {
    do { if (!((diagId == DIAG_ID_CURRENT_SENSOR_V1_MEASUREMENT_TIMEOUT) || (diagId == DIAG_ID_CURRENT_SENSOR_V2_MEASUREMENT_TIMEOUT) || (diagId == DIAG_ID_CURRENT_SENSOR_V3_MEASUREMENT_TIMEOUT))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 148); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!((event == DIAG_EVENT_OK) || (event == DIAG_EVENT_NOT_OK) || (event == DIAG_EVENT_RESET))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 149); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(kpkDiagShim != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 150); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(stringNumber < (1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 151); } while (0); FAS_InfiniteLoop(); } } while (0);

    switch (diagId) {
        case DIAG_ID_CURRENT_SENSOR_V1_MEASUREMENT_TIMEOUT:
            if (event == DIAG_EVENT_RESET) {
                kpkDiagShim->pTableError->currentSensorVoltage1TimeoutError[stringNumber] = 0;
            } else if (event == DIAG_EVENT_NOT_OK) {
                kpkDiagShim->pTableError->currentSensorVoltage1TimeoutError[stringNumber] = 1;
            } else {
        ...
        case DIAG_ID_CURRENT_SENSOR_V2_MEASUREMENT_TIMEOUT:
            if (event == DIAG_EVENT_RESET) {
                kpkDiagShim->pTableError->currentSensorVoltage2TimeoutError[stringNumber] = 0;
            } else if (event == DIAG_EVENT_NOT_OK) {
                kpkDiagShim->pTableError->currentSensorVoltage2TimeoutError[stringNumber] = 1;
            } else {
        ...
        case DIAG_ID_CURRENT_SENSOR_V3_MEASUREMENT_TIMEOUT:
            if (event == DIAG_EVENT_RESET) {
                kpkDiagShim->pTableError->currentSensorVoltage3TimeoutError[stringNumber] = 0;
            } else if (event == DIAG_EVENT_NOT_OK) {
                kpkDiagShim->pTableError->currentSensorVoltage3TimeoutError[stringNumber] = 1;
            } else {
        ...
        default:
            do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 183); } while (0); FAS_InfiniteLoop(); } } while (0);
            break;  
    }
}
