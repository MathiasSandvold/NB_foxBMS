...
volatile _Bool ftsk_allQueuesCreated = 0;

QueueHandle_t ftsk_databaseQueue = ((void *)(0u));

QueueHandle_t ftsk_imdCanDataQueue = ((void *)(0u));

 
QueueHandle_t ftsk_canRxQueue = ((void *)(0u));
 

QueueHandle_t ftsk_afeRequestQueue;
QueueHandle_t ftsk_rtcSetTimeQueue = ((void *)(0u));

QueueHandle_t ftsk_afeToI2cQueue   = ((void *)(0u));
QueueHandle_t ftsk_afeFromI2cQueue = ((void *)(0u));
...
extern void FTSK_CreateQueues(void) {
    ...
    static uint8_t ftsk_databaseQueueStorageArea[((1u) * (sizeof(DATA_QUEUE_MESSAGE_s)))] = {0};
    static StaticQueue_t ftsk_databaseQueueStructure                               = {0};

    
 
    ftsk_databaseQueue = MPU_xQueueGenericCreateStatic( ( (1u) ), ( (sizeof(DATA_QUEUE_MESSAGE_s)) ), ( ftsk_databaseQueueStorageArea ), ( &ftsk_databaseQueueStructure ), ( ( ( uint8_t ) 0U ) ) );
    do { if (!(ftsk_databaseQueue != ((void *)0u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 147); } while (0); FAS_InfiniteLoop(); } } while (0);
    ;

     
    static uint8_t ftsk_imdQueueStorageArea[((5u) * (sizeof(CAN_BUFFER_ELEMENT_s)))] = {0};
    static StaticQueue_t ftsk_imdQueueStructure                          = {0};

    ftsk_imdCanDataQueue = MPU_xQueueGenericCreateStatic( ( (5u) ), ( (sizeof(CAN_BUFFER_ELEMENT_s)) ), ( ftsk_imdQueueStorageArea ), ( &ftsk_imdQueueStructure ), ( ( ( uint8_t ) 0U ) ) );
    ;
    do { if (!(ftsk_imdCanDataQueue != ((void *)0u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 157); } while (0); FAS_InfiniteLoop(); } } while (0);

     
     
    static uint8_t ftsk_canRxQueueStorageArea[((50u) * (sizeof(CAN_BUFFER_ELEMENT_s)))] = {0};
    static StaticQueue_t ftsk_canRxQueueStructure                             = {0};
     

    ftsk_canRxQueue = MPU_xQueueGenericCreateStatic( ( (50u) ), ( (sizeof(CAN_BUFFER_ELEMENT_s)) ), ( ftsk_canRxQueueStorageArea ), ( &ftsk_canRxQueueStructure ), ( ( ( uint8_t ) 0U ) ) );
    ;
    do { if (!(ftsk_canRxQueue != ((void *)0u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 171); } while (0); FAS_InfiniteLoop(); } } while (0);
    ...
    static uint8_t ftsk_afeRequestQueueStorageArea[(1u) * (sizeof(AFE_REQUEST_e))] = {
        0};
    static StaticQueue_t ftsk_afeRequestQueueStructure = {0};  

    
 
    ftsk_afeRequestQueue = MPU_xQueueGenericCreateStatic( ( (1u) ), ( (sizeof(AFE_REQUEST_e)) ), ( ftsk_afeRequestQueueStorageArea ), ( &ftsk_afeRequestQueueStructure ), ( ( ( uint8_t ) 0U ) ) );
    do { if (!(ftsk_afeRequestQueue != ((void *)0u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 190); } while (0); FAS_InfiniteLoop(); } } while (0);
    ;

     
    static uint8_t ftsk_rtcQueueStorageArea[((1u) * (sizeof(RTC_TIME_DATA_s)))] = {0};
    static StaticQueue_t ftsk_rtcQueueStructure                          = {0};

    ftsk_rtcSetTimeQueue = MPU_xQueueGenericCreateStatic( ( (1u) ), ( (sizeof(RTC_TIME_DATA_s)) ), ( ftsk_rtcQueueStorageArea ), ( &ftsk_rtcQueueStructure ), ( ( ( uint8_t ) 0U ) ) );
    ;
    do { if (!(ftsk_rtcSetTimeQueue != ((void *)0u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 200); } while (0); FAS_InfiniteLoop(); } } while (0);

     
    static uint8_t ftsk_afeToI2cQueueStorageArea[((1u) * (sizeof(AFE_I2C_QUEUE_s)))] = {0};
    static StaticQueue_t ftsk_afeToI2cQueueStructure                             = {0};

    ftsk_afeToI2cQueue = MPU_xQueueGenericCreateStatic( ( (1u) ), ( (sizeof(AFE_I2C_QUEUE_s)) ), ( ftsk_afeToI2cQueueStorageArea ), ( &ftsk_afeToI2cQueueStructure ), ( ( ( uint8_t ) 0U ) ) );
    ;
    do { if (!(ftsk_afeToI2cQueue != ((void *)0u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 212); } while (0); FAS_InfiniteLoop(); } } while (0);

     
    static uint8_t ftsk_afeFromI2cQueueStorageArea[((1u) * (sizeof(AFE_I2C_QUEUE_s)))] = {0};
    static StaticQueue_t ftsk_afeFromI2cQueueStructure                             = {0};

    ftsk_afeFromI2cQueue = MPU_xQueueGenericCreateStatic( ( (1u) ), ( (sizeof(AFE_I2C_QUEUE_s)) ), ( ftsk_afeFromI2cQueueStorageArea ), ( &ftsk_afeFromI2cQueueStructure ), ( ( ( uint8_t ) 0U ) ) );
    ;
    do { if (!(ftsk_afeFromI2cQueue != ((void *)0u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 224); } while (0); FAS_InfiniteLoop(); } } while (0);
...
extern void FTSK_CreateTasks(void) {
    ...
    static StackType_t ftsk_stackEngine[(((1024u)) / (4u))] = {0};

    const TaskHandle_t ftsk_taskHandleEngine = MPU_xTaskCreateStatic(
        (TaskFunction_t)FTSK_CreateTaskEngine,
        (const char *)"TaskEngine",
        ((ftsk_taskDefinitionEngine . stackSize_B) / (4u)),
        (void *)ftsk_taskDefinitionEngine.pvParameters,
        (UBaseType_t)ftsk_taskDefinitionEngine.priority,
        ftsk_stackEngine,
        &ftsk_taskEngine);
    do { if (!(ftsk_taskHandleEngine != ((void *)0u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 250); } while (0); FAS_InfiniteLoop(); } } while (0);  

     
    static StaticTask_t ftsk_taskCyclic1ms                                           = {0};
    static StackType_t ftsk_stackCyclic1ms[(((1024u)) / (4u))] = {0};

    const TaskHandle_t ftsk_taskHandleCyclic1ms = MPU_xTaskCreateStatic(
        (TaskFunction_t)FTSK_CreateTaskCyclic1ms,
        (const char *)"TaskCyclic1ms",
        ((ftsk_taskDefinitionCyclic1ms . stackSize_B) / (4u)),
        (void *)ftsk_taskDefinitionCyclic1ms.pvParameters,
        (UBaseType_t)ftsk_taskDefinitionCyclic1ms.priority,
        ftsk_stackCyclic1ms,
        &ftsk_taskCyclic1ms);
    do { if (!(ftsk_taskHandleCyclic1ms != ((void *)0u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 264); } while (0); FAS_InfiniteLoop(); } } while (0);  

     
    static StaticTask_t ftsk_taskCyclic10ms                                            = {0};
    static StackType_t ftsk_stackCyclic10ms[(((5120u)) / (4u))] = {0};

    const TaskHandle_t ftsk_taskHandleCyclic10ms = MPU_xTaskCreateStatic(
        (TaskFunction_t)FTSK_CreateTaskCyclic10ms,
        (const char *)"TaskCyclic10ms",
        ((ftsk_taskDefinitionCyclic10ms . stackSize_B) / (4u)),
        (void *)ftsk_taskDefinitionCyclic10ms.pvParameters,
        (UBaseType_t)ftsk_taskDefinitionCyclic10ms.priority,
        ftsk_stackCyclic10ms,
        &ftsk_taskCyclic10ms);
    do { if (!(ftsk_taskHandleCyclic10ms != ((void *)0u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 278); } while (0); FAS_InfiniteLoop(); } } while (0);  

     
    static StaticTask_t ftsk_taskCyclic100ms                                             = {0};
    static StackType_t ftsk_stackCyclic100ms[(((1024u)) / (4u))] = {0};

    const TaskHandle_t ftsk_taskHandleCyclic100ms = MPU_xTaskCreateStatic(
        (TaskFunction_t)FTSK_CreateTaskCyclic100ms,
        (const char *)"TaskCyclic100ms",
        ((ftsk_taskDefinitionCyclic100ms . stackSize_B) / (4u)),
    ...
    do { if (!(ftsk_taskHandleCyclic100ms != ((void *)0u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 292); } while (0); FAS_InfiniteLoop(); } } while (0);  

     
    static StaticTask_t ftsk_taskCyclicAlgorithm100ms                                                       = {0};
    static StackType_t ftsk_stackCyclicAlgorithm100ms[(((1024u)) / (4u))] = {0};

    const TaskHandle_t ftsk_taskHandleCyclicAlgorithm100ms = MPU_xTaskCreateStatic(
        (TaskFunction_t)FTSK_CreateTaskCyclicAlgorithm100ms,
        (const char *)"TaskCyclicAlgorithm100ms",
        ((ftsk_taskDefinitionCyclicAlgorithm100ms . stackSize_B) / (4u)),
    ...
    do { if (!(ftsk_taskHandleCyclicAlgorithm100ms != ((void *)0u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 306); } while (0); FAS_InfiniteLoop(); } } while (0);  

     
    static StaticTask_t ftsk_taskI2c                                        = {0};
    static StackType_t ftsk_stackSizeI2c[(((2048u / 4u)) / (4u))] = {0};

    ftsk_taskHandleI2c = MPU_xTaskCreateStatic(
        (TaskFunction_t)FTSK_CreateTaskI2c,
        (const char *)"TaskI2c",
        ((ftsk_taskDefinitionI2c . stackSize_B) / (4u)),
        (void *)ftsk_taskDefinitionI2c.pvParameters,
        (UBaseType_t)ftsk_taskDefinitionI2c.priority,
        ftsk_stackSizeI2c,
        &ftsk_taskI2c);
    do { if (!(ftsk_taskHandleI2c != ((void *)0u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 320); } while (0); FAS_InfiniteLoop(); } } while (0);  
    ...
    static StackType_t ftsk_stackSizeAfe[(((2048u / 4u)) / (4u))] = {0};

    ftsk_taskHandleAfe = MPU_xTaskCreateStatic(
        (TaskFunction_t)FTSK_CreateTaskAfe,
        (const char *)"TaskAfe",
        ((ftsk_taskDefinitionAfe . stackSize_B) / (4u)),
        (void *)ftsk_taskDefinitionAfe.pvParameters,
        (UBaseType_t)ftsk_taskDefinitionAfe.priority,
        ftsk_stackSizeAfe,
        &ftsk_taskAfe);
    do { if (!(ftsk_taskHandleAfe != ((void *)0u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 338); } while (0); FAS_InfiniteLoop(); } } while (0);  
}
