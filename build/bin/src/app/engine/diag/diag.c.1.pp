...
STD_RETURN_TYPE_e DIAG_Initialize(DIAG_DEV_s *diag_dev_pointer) {
    do { if (!(diag_dev_pointer != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 117); } while (0); FAS_InfiniteLoop(); } } while (0);

    STD_RETURN_TYPE_e retval = STD_OK;
    uint8_t id_nr            = (uint8_t)DIAG_ID_MAX;
     
    do { if (!((uint16_t)DIAG_ID_MAX < 0xff)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 122); } while (0); FAS_InfiniteLoop(); } } while (0);
    ...
    for (uint16_t diagnosisEntry = 0u; diagnosisEntry < diag_dev_pointer->nrOfConfiguredDiagnosisEntries;
         diagnosisEntry++) {
        _Bool fatalErrorDetected = (_Bool)(diag_diagnosisIdConfiguration[diagnosisEntry].severity == DIAG_FATAL_ERROR);
        _Bool discardDelay = (_Bool)(diag_diagnosisIdConfiguration[diagnosisEntry].delay_ms == (0xffffffffU));
        if (fatalErrorDetected && discardDelay) {
             
            do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 185); } while (0); FAS_InfiniteLoop(); } } while (0);
        }
    }
...
static uint8_t DIAG_EntryWrite(uint8_t eventID, DIAG_EVENT_e event, uint32_t data) {
    ...
    if (++diag.entry_cnt[eventID] > (5)) {
         
        diag.entry_cnt[eventID] = (5);
        return ret_val;
    }
...
DIAG_RETURNTYPE_e DIAG_Handler(DIAG_ID_e diagId, DIAG_EVENT_e event, DIAG_IMPACT_LEVEL_e impact, uint32_t data) {
    ...
    uint32_t *u32ptr_errCodemsk    = ((void *)(0u));
    uint32_t *u32ptr_warnCodemsk   = ((void *)(0u));
    uint16_t *u16ptr_threshcounter = ((void *)(0u));
    ...
    err_enable_idx     = diagId / 32;         
    err_enable_bitmask = 1 << (diagId % 32);  
    ...
    if (event == DIAG_EVENT_OK) {
        if ((diag.err_enableflag[err_enable_idx] & err_enable_bitmask) > 0u) {
            ...
            if (((*u16ptr_threshcounter) == 0)) {
                ...
                *u32ptr_errCodemsk &= ~err_enable_bitmask;   
                *u32ptr_warnCodemsk &= ~err_enable_bitmask;  
        ...
        ret_val = DIAG_HANDLER_RETURN_OK;  
        ...
        if ((diag.err_enableflag[err_enable_idx] & err_enable_bitmask) > 0u) {
            ...
            *u32ptr_errCodemsk &= ~err_enable_bitmask;       
            *u32ptr_warnCodemsk |= err_enable_bitmask;       
            ret_val = DIAG_HANDLER_RETURN_WARNING_OCCURRED;  
        }
...
uint32_t DIAG_GetDelay(DIAG_ID_e diagnosisEntry) {
    do { if (!(diagnosisEntry < DIAG_ID_MAX)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 387); } while (0); FAS_InfiniteLoop(); } } while (0);
    return diag_diagnosisIdConfiguration[diag.id2ch[(uint16_t)diagnosisEntry]].delay_ms;
}

_Bool DIAG_IsAnyFatalErrorSet(void) {
