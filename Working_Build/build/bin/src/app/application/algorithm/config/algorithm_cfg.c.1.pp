...
ALGO_TASKS_s algo_algorithms[] = {
    {ALGO_UNINITIALIZED, 100, 1000, 0, ((void *)(0u)), &ALGO_MovAverage},
};
...
extern void ALGO_MarkAsDone(uint32_t algorithmIndex) {
    do { if (!(algorithmIndex < algo_length)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 83); } while (0); FAS_InfiniteLoop(); } } while (0);
...
extern void ALGO_MarkAsReinit(uint32_t algorithmIndex) {
    do { if (!(algorithmIndex < algo_length)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 94); } while (0); FAS_InfiniteLoop(); } } while (0);
