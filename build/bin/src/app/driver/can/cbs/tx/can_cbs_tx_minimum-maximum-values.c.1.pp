...
extern uint32_t CellMeasurements(
    ...
    uint8_t *pMuxId,
    const CAN_SHIM_s *const kpkCanShim) {
    do { if (!(message . id == (0x71Cu))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 251); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(message . idType == (CAN_STANDARD_IDENTIFIER_11_BIT))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 252); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(message . dlc == (8u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 253); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pCanData != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 254); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pMuxId == ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 255); } while (0); FAS_InfiniteLoop(); } } while (0);  
    do { if (!(kpkCanShim != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 256); } while (0); FAS_InfiniteLoop(); } } while (0);
    uint64_t messageData = 0u;
