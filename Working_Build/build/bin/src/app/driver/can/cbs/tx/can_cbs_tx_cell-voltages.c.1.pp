...
static const CAN_SIGNAL_TYPE_s cantx_cellVoltageMultiplexer  = {0u, 8u, 1.0f, 0.0f, 0.0f, 1.0f};
static const CAN_SIGNAL_TYPE_s cantx_cellVoltage0InvalidFlag = {8u, 1u, 1.0f, 0.0f, 0.0f, 1.0f};
static const CAN_SIGNAL_TYPE_s cantx_cellVoltage1InvalidFlag = {8u, 1u, 1.0f, 0.0f, 0.0f, 1.0f};
static const CAN_SIGNAL_TYPE_s cantx_cellVoltage2InvalidFlag = {10u, 1u, 1.0f, 0.0f, 0.0f, 1.0f};
static const CAN_SIGNAL_TYPE_s cantx_cellVoltage3InvalidFlag = {11u, 1u, 1.0f, 0.0f, 0.0f, 1.0f};
static const CAN_SIGNAL_TYPE_s cantx_cellVoltage0_mV         = {12u, 13u, 1.0f, 0.0f, 0.0f, 8192.0f};
static const CAN_SIGNAL_TYPE_s cantx_cellVoltage1_mV         = {25u, 13u, 1.0f, 0.0f, 0.0f, 8192.0f};
static const CAN_SIGNAL_TYPE_s cantx_cellVoltage2_mV         = {38u, 13u, 1.0f, 0.0f, 0.0f, 8192.0f};
static const CAN_SIGNAL_TYPE_s cantx_cellVoltage3_mV         = {51u, 13u, 1.0f, 0.0f, 0.0f, 8192.0f};
...
extern uint32_t CANTX_CellVoltages(
    ...
    uint8_t *pMuxId,
    const CAN_SHIM_s *const kpkCanShim) {
    do { if (!(message . id == (0x240u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 160); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(message . idType == (CAN_STANDARD_IDENTIFIER_11_BIT))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 161); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(message . dlc == (8u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 162); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pCanData != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 163); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pMuxId != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 164); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(kpkCanShim != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 165); } while (0); FAS_InfiniteLoop(); } } while (0);
    ...
    if (*pMuxId == 0u) {
        (DATA_Read1DataBlock) (kpkCanShim->pTableCellVoltage);
    }

     
    uint32_t signalData = *pMuxId / (4u);
    CAN_TxSetMessageDataWithSignalData(
        &messageData,
