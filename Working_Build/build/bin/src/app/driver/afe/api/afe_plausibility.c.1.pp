...
extern STD_RETURN_TYPE_e AFE_PlausibilityCheckVoltageMeasurementRange(
    ...
    do { if (!(plausibleValues . maximumPlausibleVoltage_mV > plausibleValues . minimumPlausibleVoltage_mV)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 79); } while (0); FAS_InfiniteLoop(); } } while (0);
...
extern STD_RETURN_TYPE_e AFE_PlausibilityCheckTempMinMax(const int16_t celltemperature_ddegC) {
    ...
    do { if (!(plausibleMaximumTemperature_ddegC >= plausibleMinimumTemperature_ddegC)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 96); } while (0); FAS_InfiniteLoop(); } } while (0);
