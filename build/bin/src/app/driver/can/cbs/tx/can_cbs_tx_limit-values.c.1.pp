...
extern uint32_t CANTX_LimitValues(
    ...
    do { if (!(pMuxId == ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 82); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(message . id == (0x351u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 83); } while (0); FAS_InfiniteLoop(); } } while (0);
    ...
    do { if (!(pCanData != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 86); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(kpkCanShim != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 87); } while (0); FAS_InfiniteLoop(); } } while (0);
    uint64_t messageData = 0u;

    (DATA_Read1DataBlock) (kpkCanShim->pTableSof);

     
     
    float_t signalData = (float_t)kpkCanShim->pTableSof->recommendedContinuousPackChargeCurrent_mA;
    float_t offset     = 0.0f;
    float_t factor     = 0.004f;  
    ...
    signalData = (float_t)((16u) * (3488));
    offset     = 0.0f;
    factor     = 0.01f;  
    signalData = (signalData + offset) * factor;
    data       = (int64_t)signalData;
     
    CAN_TxSetMessageDataWithSignalData(&messageData, 0u, 18u, data, message.endianness);
...
extern uint32_t BatteryLimits(
    ...
    do { if (!(message . id == (0x351u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 146); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(message . idType == (CAN_STANDARD_IDENTIFIER_11_BIT))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 147); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(message . dlc == (8u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 148); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pCanData != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 149); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(kpkCanShim != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 150); } while (0); FAS_InfiniteLoop(); } } while (0);
    uint64_t messageData = 0u;
...
extern uint32_t ModuleLimits(
    ...
    do { if (!(pMuxId == ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 196); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(message . id == (0x71Du))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 197); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(message . idType == (CAN_STANDARD_IDENTIFIER_11_BIT))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 198); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(message . dlc == (8u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 199); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pCanData != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 200); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(kpkCanShim != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 201); } while (0); FAS_InfiniteLoop(); } } while (0);
    uint64_t messageData = 0u;
