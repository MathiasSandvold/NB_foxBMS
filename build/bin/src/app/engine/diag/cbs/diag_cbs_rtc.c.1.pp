...
extern void DIAG_Rtc(
    ...
    const DIAG_DATABASE_SHIM_s *const kpkDiagShim,
    uint32_t data) {
    do { if (!((diagId == DIAG_ID_RTC_CLOCK_INTEGRITY_ERROR) || (diagId == DIAG_ID_RTC_BATTERY_LOW_ERROR))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 76); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!((event == DIAG_EVENT_OK) || (event == DIAG_EVENT_NOT_OK) || (event == DIAG_EVENT_RESET))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 77); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(kpkDiagShim != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 78); } while (0); FAS_InfiniteLoop(); } } while (0);
    (void)data;  

    if (diagId == DIAG_ID_RTC_CLOCK_INTEGRITY_ERROR) {
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->rtcClockIntegrityError = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->rtcClockIntegrityError = 1;
        }
    }

    if (diagId == DIAG_ID_RTC_BATTERY_LOW_ERROR) {
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->rtcBatteryLowError = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->rtcBatteryLowError = 1;
        }
    }
}
