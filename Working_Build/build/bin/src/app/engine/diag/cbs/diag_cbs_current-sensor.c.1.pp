...
void DIAG_ErrorCurrentSensor(
    ...
    if (diagId == DIAG_ID_CURRENT_SENSOR_RESPONDING) {
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->currentSensorNotRespondingError[stringNumber] = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->currentSensorNotRespondingError[stringNumber] = 1;
        }
    } else if (diagId == DIAG_ID_CAN_CC_RESPONDING) {
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->currentSensorCoulombCounterTimeoutError[stringNumber] = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->currentSensorCoulombCounterTimeoutError[stringNumber] = 1;
        }
    } else if (diagId == DIAG_ID_CAN_EC_RESPONDING) {
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->currentSensorEnergyCounterTimeoutError[stringNumber] = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->currentSensorEnergyCounterTimeoutError[stringNumber] = 1;
        }
    }
}
