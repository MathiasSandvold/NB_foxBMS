...
static void CONT_GetFeedbackOfAllContactors(void) {
    ...
    for (CONT_CONTACTOR_INDEX contactor = 0; contactor < ((2u * (1u)) + (1u)); contactor++) {
        ...
        if (cont_contactorStates[contactor].feedbackPinType == CONT_HAS_NO_FEEDBACK) {
            ...
            cont_contactorStates[contactor].feedback =
                SPS_GetChannelPexFeedback(cont_contactorStates[contactor].spsChannel, 1);
        } else {
             
            cont_contactorStates[contactor].feedback =
                SPS_GetChannelPexFeedback(cont_contactorStates[contactor].spsChannel, 0);
        }
    }
}
...
static void CONT_InitializationCheckOfContactorRegistry(void) {
    ...
    for (CONT_CONTACTOR_INDEX contactor = 0u; contactor < ((2u * (1u)) + (1u)); contactor++) {
         

         
        do { if (!(cont_contactorStates[contactor]. spsChannel < ((4u) * (2u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 115); } while (0); FAS_InfiniteLoop(); } } while (0);

         
        const SPS_CHANNEL_AFFILIATION_e channelAffiliation =
            SPS_GetChannelAffiliation(cont_contactorStates[contactor].spsChannel);
        do { if (!(SPS_AFF_CONTACTOR == channelAffiliation)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 120); } while (0); FAS_InfiniteLoop(); } } while (0);
    }
}
    ...
    for (CONT_CONTACTOR_INDEX contactor = 0u; contactor < ((2u * (1u)) + (1u)); contactor++) {
        ...
        switch (cont_contactorStates[contactor].type) {
            ...
            default:
                ...
                do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 160); } while (0); FAS_InfiniteLoop(); } } while (0);
        }
    }
}

extern STD_RETURN_TYPE_e CONT_OpenContactor(uint8_t stringNumber, CONT_TYPE_e contactor) {
    do { if (!(stringNumber < (1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 166); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(contactor != CONT_UNDEFINED)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 167); } while (0); FAS_InfiniteLoop(); } } while (0);
    STD_RETURN_TYPE_e retval = STD_NOT_OK;
     
    for (uint8_t contactorIndex = 0u; contactorIndex < ((2u * (1u)) + (1u)); contactorIndex++) {
...
extern STD_RETURN_TYPE_e CONT_CloseContactor(uint8_t stringNumber, CONT_TYPE_e contactor) {
    do { if (!(stringNumber < (1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 185); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(contactor != CONT_UNDEFINED)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 186); } while (0); FAS_InfiniteLoop(); } } while (0);
    STD_RETURN_TYPE_e retval = STD_NOT_OK;
...
extern STD_RETURN_TYPE_e CONT_ClosePrecharge(uint8_t stringNumber) {
    do { if (!(stringNumber < (1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 204); } while (0); FAS_InfiniteLoop(); } } while (0);
    STD_RETURN_TYPE_e retVal = STD_NOT_OK;
...
extern STD_RETURN_TYPE_e CONT_OpenPrecharge(uint8_t stringNumber) {
    do { if (!(stringNumber < (1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 214); } while (0); FAS_InfiniteLoop(); } } while (0);
    STD_RETURN_TYPE_e retVal = STD_NOT_OK;
...
extern CONT_ELECTRICAL_STATE_TYPE_e CONT_GetContactorState(uint8_t stringNumber, CONT_TYPE_e contactorType) {
    do { if (!(stringNumber < (1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 237); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(contactorType != CONT_UNDEFINED)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 238); } while (0); FAS_InfiniteLoop(); } } while (0);
