...
extern void SOA_CheckVoltages(DATA_BLOCK_MIN_MAX_s *pMinimumMaximumCellVoltages) {
    do { if (!(pMinimumMaximumCellVoltages != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 82); } while (0); FAS_InfiniteLoop(); } } while (0);
...
extern void SOA_CheckTemperatures(
    DATA_BLOCK_MIN_MAX_s *pMinimumMaximumCellTemperatures,
    DATA_BLOCK_PACK_VALUES_s *pCurrent) {
    do { if (!(pMinimumMaximumCellTemperatures != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 151); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pCurrent != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 152); } while (0); FAS_InfiniteLoop(); } } while (0);
...
extern void SOA_CheckCurrent(DATA_BLOCK_PACK_VALUES_s *pTablePackValues) {
    do { if (!(pTablePackValues != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 272); } while (0); FAS_InfiniteLoop(); } } while (0);
    ...
    for (uint8_t s = 0u; s < (1u); s++) {
        ...
        if (pTablePackValues->invalidStringCurrent[s] == 0u) {
            ...
            _Bool stringOvercurrent = SOA_IsStringCurrentLimitViolated(absStringCurrent_mA, currentDirection);
            _Bool cellOvercurrent   = SOA_IsCellCurrentLimitViolated(absStringCurrent_mA, currentDirection);
            if (currentDirection == BMS_CHARGING) {
                 
                if (stringOvercurrent == 1) {
            ...
            if (SOA_IsCurrentOnOpenString(currentDirection, s) == 0) {
    ...
    if (pTablePackValues->invalidPackCurrent == 0u) {
        ...
        _Bool packOvercurrent                      = SOA_IsPackCurrentLimitViolated(absPackCurrent_mA, currentDirection);

        if (currentDirection == BMS_CHARGING) {
            if (packOvercurrent == 1) {
