...
extern void OS_IncrementTimer(void) {
    ...
    if (++os_timer.timer_1ms > 9u) {  
        os_timer.timer_1ms = 0u;
        if (++os_timer.timer_10ms > 9u) {  
            os_timer.timer_10ms = 0u;
            if (++os_timer.timer_100ms > 9u) {  
                os_timer.timer_100ms = 0u;
                if (++os_timer.timer_sec > 59u) {  
                    os_timer.timer_sec = 0u;
                    if (++os_timer.timer_min > 59u) {  
                        os_timer.timer_min = 0u;
                        if (++os_timer.timer_h > 23u) {  
                            os_timer.timer_h = 0u;
                            ++os_timer.timer_d;
                        }
                    }
...
extern STD_RETURN_TYPE_e OS_CheckTimeHasPassedSelfTest(void) {
    ...
    if (OS_CheckTimeHasPassedWithTimestamp(0u, 0u, 0u) != 1) {
        
 
        selfCheckReturnValue = STD_NOT_OK;
    }

    if (OS_CheckTimeHasPassedWithTimestamp(0u, 1u, 1u) != 1) {
        
 
        selfCheckReturnValue = STD_NOT_OK;
    }

    if (OS_CheckTimeHasPassedWithTimestamp(1u, 2u, 2u) != 0) {
        
 
        selfCheckReturnValue = STD_NOT_OK;
    }

    if (OS_CheckTimeHasPassedWithTimestamp(0u, 1u, 0u) != 1) {
        
 
        selfCheckReturnValue = STD_NOT_OK;
    }

    if (OS_CheckTimeHasPassedWithTimestamp(1u, 0u, 1u) != 1) {
        selfCheckReturnValue = STD_NOT_OK;
    }
