...
static uint8_t pex_inputPort0[(3u)]           = {0};
static uint8_t pex_inputPort1[(3u)]           = {0};
static uint8_t pex_outputPort0[(3u)]          = {0};
static uint8_t pex_outputPort1[(3u)]          = {0};
static uint8_t pex_configPolarityPort0[(3u)]  = {0};
static uint8_t pex_configPolarityPort1[(3u)]  = {0};
static uint8_t pex_configDirectionPort0[(3u)] = {0};
static uint8_t pex_configDirectionPort1[(3u)] = {0};
...
static uint8_t pex_inputPort0Local[(3u)]           = {0};
static uint8_t pex_inputPort1Local[(3u)]           = {0};
static uint8_t pex_outputPort0Local[(3u)]          = {0};
static uint8_t pex_outputPort1Local[(3u)]          = {0};
static uint8_t pex_configPolarityPort0Local[(3u)]  = {0};
static uint8_t pex_configPolarityPort1Local[(3u)]  = {0};
static uint8_t pex_configDirectionPort0Local[(3u)] = {0};
static uint8_t pex_configDirectionPort1Local[(3u)] = {0};
...
static STD_RETURN_TYPE_e PEX_ReadInputs(void) {
    ...
    for (uint8_t i = 0u; i < (3u); i++) {
        ...
        pex_i2cDataWrite[0u] = (0x0u);
        pex_i2cDataRead[0u]  = 0u;
        pex_i2cDataRead[1u]  = 0u;
        STD_RETURN_TYPE_e i2cReadReturn =
            I2C_WriteReadDma((((i2cBASE_t *)0xFFF7D400U)), pex_addressList[i], 1u, pex_i2cDataWrite, 2u, pex_i2cDataRead);
...
static STD_RETURN_TYPE_e PEX_WriteOutputs(void) {
    ...
    for (uint8_t i = 0u; i < (3u); i++) {
        ...
        pex_i2cDataWrite[0u]             = (0x2u);
        pex_i2cDataWrite[1u]             = pex_outputPort0Local[i];
        pex_i2cDataWrite[2u]             = pex_outputPort1Local[i];
        STD_RETURN_TYPE_e i2cWriteReturn = I2C_WriteDma((((i2cBASE_t *)0xFFF7D400U)), pex_addressList[i], 3u, pex_i2cDataWrite);
...
extern void PEX_Initialize(void) {
    for (uint8_t i = 0u; i < (3u); i++) {
        ...
        pex_outputPort0[i]      = (0xFFu);
        pex_outputPort1[i]      = (0xFFu);
        pex_outputPort0Local[i] = (0xFFu);
        pex_outputPort1Local[i] = (0xFFu);
        

 
        pex_configPolarityPort0[i]      = (0x0u);
        pex_configPolarityPort1[i]      = (0x0u);
        pex_configPolarityPort0Local[i] = (0x0u);
        pex_configPolarityPort1Local[i] = (0x0u);
         
        pex_configDirectionPort0[i]      = (0xFFu);
        pex_configDirectionPort1[i]      = (0xFFu);
        pex_configDirectionPort0Local[i] = (0xFFu);
        pex_configDirectionPort1Local[i] = (0xFFu);
    }
}
...
extern void PEX_SetPin(uint8_t portExpander, uint8_t pin) {
    do { if (!(portExpander < (3u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 337); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pin <= (15u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 338); } while (0); FAS_InfiniteLoop(); } } while (0);

    if (pin <= (7u)) {
        OS_EnterTaskCritical();
        pex_outputPort0[portExpander] |= (1u) << pin;
        OS_ExitTaskCritical();
    } else {  
        OS_EnterTaskCritical();
        pex_outputPort1[portExpander] |= (1u) << (pin % (8u));
        OS_ExitTaskCritical();
    }
}

extern void PEX_ResetPin(uint8_t portExpander, uint8_t pin) {
    do { if (!(portExpander < (3u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 352); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pin <= (15u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 353); } while (0); FAS_InfiniteLoop(); } } while (0);
...
extern uint8_t PEX_GetPin(uint8_t portExpander, uint8_t pin) {
    do { if (!(portExpander < (3u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 367); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pin <= (15u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 368); } while (0); FAS_InfiniteLoop(); } } while (0);
    ...
    if (pin <= (7u)) {
        ...
        OS_EnterTaskCritical();
        pinState = (pex_inputPort1[portExpander] >> (pin % (8u))) & 0x1u;
        OS_ExitTaskCritical();
    }
    ...
    if (pinState == 0u) {
        ...
        do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 388); } while (0); FAS_InfiniteLoop(); } } while (0);
    }
...
extern void PEX_SetPinDirectionInput(uint8_t portExpander, uint8_t pin) {
    do { if (!(portExpander < (3u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 395); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pin <= (15u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 396); } while (0); FAS_InfiniteLoop(); } } while (0);

    if (pin <= (7u)) {
        OS_EnterTaskCritical();
        pex_configDirectionPort0[portExpander] |= (1u) << pin;
        OS_ExitTaskCritical();
    } else {  
        OS_EnterTaskCritical();
        pex_configDirectionPort1[portExpander] |= (1u) << (pin % (8u));
        OS_ExitTaskCritical();
    }
}

extern void PEX_SetPinDirectionOutput(uint8_t portExpander, uint8_t pin) {
    do { if (!(portExpander < (3u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 410); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pin <= (15u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 411); } while (0); FAS_InfiniteLoop(); } } while (0);

    if (pin <= (7u)) {
        OS_EnterTaskCritical();
        pex_configDirectionPort0[portExpander] &= ~(((~(0u)) & 0x1u) << pin);
        OS_ExitTaskCritical();
    } else {  
        OS_EnterTaskCritical();
...
extern void PEX_SetPinPolarityInverted(uint8_t portExpander, uint8_t pin) {
    do { if (!(portExpander < (3u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 426); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pin <= (15u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 427); } while (0); FAS_InfiniteLoop(); } } while (0);

    if (pin <= (7u)) {
        OS_EnterTaskCritical();
        pex_configPolarityPort0[portExpander] |= (1u) << pin;
        OS_ExitTaskCritical();
    } else {  
        OS_EnterTaskCritical();
        pex_configPolarityPort1[portExpander] |= (1u) << (pin % (8u));
        OS_ExitTaskCritical();
    }
}

extern void PEX_SetPinPolarityRetained(uint8_t portExpander, uint8_t pin) {
    do { if (!(portExpander < (3u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 441); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(pin <= (15u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 442); } while (0); FAS_InfiniteLoop(); } } while (0);

    if (pin <= (7u)) {
        OS_EnterTaskCritical();
        pex_configPolarityPort0[portExpander] &= ~(((~(0u)) & 0x1u) << pin);
        OS_ExitTaskCritical();
    } else {  
        OS_EnterTaskCritical();
