...
static const uint8_t can_bigEndianTable[(64u)] = {
...
PERSISTANT_ERROR_FLAG_s persistant_error_flags[11u] = {
    {.latched_value = 0, .can_offset = 29, .id = DIAG_ID_CELL_VOLTAGE_OVERVOLTAGE_MSL},
    {.latched_value = 0, .can_offset = 30, .id = DIAG_ID_CELL_VOLTAGE_UNDERVOLTAGE_MSL},
    {.latched_value = 0, .can_offset = 31, .id = DIAG_ID_TEMP_OVERTEMPERATURE_CHARGE_MSL},
    {.latched_value = 0, .can_offset = 32, .id = DIAG_ID_TEMP_OVERTEMPERATURE_DISCHARGE_MSL},
    {.latched_value = 0, .can_offset = 33, .id = DIAG_ID_TEMP_UNDERTEMPERATURE_CHARGE_MSL},
    {.latched_value = 0, .can_offset = 34, .id = DIAG_ID_TEMP_UNDERTEMPERATURE_DISCHARGE_MSL},
    {.latched_value = 0, .can_offset = 35, .id = DIAG_ID_OVERCURRENT_DISCHARGE_CELL_MSL},
    {.latched_value = 0, .can_offset = 36, .id = DIAG_ID_OVERCURRENT_CHARGE_CELL_MSL},
    {.latched_value = 0, .can_offset = 37, .id = DIAG_ID_DEEP_DISCHARGE_DETECTED},
    {.latched_value = 0, .can_offset = 38, .id = DIAG_ID_AFE_COM_INTEGRITY},
    {.latched_value = 0, .can_offset = 39, .id = DIAG_ID_PLAUSIBILITY_PACK_VOLTAGE},
};
...
static uint64_t CAN_ConvertBitStartBigEndian(uint64_t bitStart, uint64_t bitLength) {
    ...
    do { if (!(bitStart < (64u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 119); } while (0); FAS_InfiniteLoop(); } } while (0);
     
    do { if (!(bitLength <= (64u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 121); } while (0); FAS_InfiniteLoop(); } } while (0);
     
    do { if (!(bitLength > 0u)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 123); } while (0); FAS_InfiniteLoop(); } } while (0);
    ...
    do { if (!(position < (64u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 151); } while (0); FAS_InfiniteLoop(); } } while (0);
    
 
    do { if (!((position + bitLength) <= (64u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 154); } while (0); FAS_InfiniteLoop(); } } while (0);
...
extern void CAN_TxPrepareSignalData(float_t *pSignal, CAN_SIGNAL_TYPE_s signalProperties) {
    do { if (!(pSignal != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 162); } while (0); FAS_InfiniteLoop(); } } while (0);
...
extern void CAN_RxConvertRawSignalData(
    ...
    float_t signalRaw,
    CAN_SIGNAL_TYPE_s signalProperties) {
    do { if (!(pSignalConverted != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 184); } while (0); FAS_InfiniteLoop(); } } while (0);
...
extern void CAN_TxSetMessageDataWithSignalData(
    ...
    uint64_t canSignal,
    CAN_ENDIANNESS_e endianness) {
    do { if (!(pMessage != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 195); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!((endianness == CAN_BIG_ENDIAN) || (endianness == CAN_LITTLE_ENDIAN))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 196); } while (0); FAS_InfiniteLoop(); } } while (0);
     
    do { if (!(bitLength <= (64u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 198); } while (0); FAS_InfiniteLoop(); } } while (0);
     
    do { if (!(bitLength > 0u)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 200); } while (0); FAS_InfiniteLoop(); } } while (0);
    ...
    do { if (!(position < (64u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 209); } while (0); FAS_InfiniteLoop(); } } while (0);
    
 
    do { if (!((position + bitLength) <= (64u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 212); } while (0); FAS_InfiniteLoop(); } } while (0);
...
extern void CAN_TxSetCanDataWithMessageData(uint64_t message, uint8_t *pCanData, CAN_ENDIANNESS_e endianness) {
    do { if (!(pCanData != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 229); } while (0); FAS_InfiniteLoop(); } } while (0);
     
    if (endianness == CAN_BIG_ENDIAN) {
        pCanData[(0u)] =
            (uint8_t)(((message) >> ((7u) * (8u))) & (0xFFu));
        pCanData[(1u)] =
            (uint8_t)(((message) >> ((6u) * (8u))) & (0xFFu));
        pCanData[(2u)] =
            (uint8_t)(((message) >> ((5u) * (8u))) & (0xFFu));
        pCanData[(3u)] =
            (uint8_t)(((message) >> ((4u) * (8u))) & (0xFFu));
        pCanData[(4u)] =
            (uint8_t)(((message) >> ((3u) * (8u))) & (0xFFu));
        pCanData[(5u)] =
            (uint8_t)(((message) >> ((2u) * (8u))) & (0xFFu));
        pCanData[(6u)] =
            (uint8_t)(((message) >> ((1u) * (8u))) & (0xFFu));
        pCanData[(7u)] =
            (uint8_t)(((message) >> ((0u) * (8u))) & (0xFFu));
        ...
        do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 267); } while (0); FAS_InfiniteLoop(); } } while (0);
    }
}
...
extern void CAN_RxGetSignalDataFromMessageData(
    ...
    uint64_t *pCanSignal,
    CAN_ENDIANNESS_e endianness) {
    do { if (!(pCanSignal != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 277); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!((endianness == CAN_BIG_ENDIAN) || (endianness == CAN_LITTLE_ENDIAN))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 278); } while (0); FAS_InfiniteLoop(); } } while (0);
     
    do { if (!(bitLength <= (64u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 280); } while (0); FAS_InfiniteLoop(); } } while (0);
     
    do { if (!(bitLength > 0u)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 282); } while (0); FAS_InfiniteLoop(); } } while (0);
     
    do { if (!(bitStart < (64u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 284); } while (0); FAS_InfiniteLoop(); } } while (0);
    uint64_t position = bitStart;
    ...
    do { if (!(position < (64u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 292); } while (0); FAS_InfiniteLoop(); } } while (0);
    
 
    do { if (!((position + bitLength) <= (64u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 295); } while (0); FAS_InfiniteLoop(); } } while (0);
...
extern void CAN_RxGetMessageDataFromCanData(
    ...
    const uint8_t *const kpkCanData,
    CAN_ENDIANNESS_e endianness) {
    do { if (!(pMessage != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 315); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(kpkCanData != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 316); } while (0); FAS_InfiniteLoop(); } } while (0);
     
    if (endianness == CAN_BIG_ENDIAN) {
        *pMessage = ((((uint64_t)kpkCanData[(0u)]) & (0xFFu))
                     << ((7u) * (8u))) |
                    ((((uint64_t)kpkCanData[(1u)]) & (0xFFu))
                     << ((6u) * (8u))) |
                    ((((uint64_t)kpkCanData[(2u)]) & (0xFFu))
                     << ((5u) * (8u))) |
                    ((((uint64_t)kpkCanData[(3u)]) & (0xFFu))
                     << ((4u) * (8u))) |
                    ((((uint64_t)kpkCanData[(4u)]) & (0xFFu))
                     << ((3u) * (8u))) |
                    ((((uint64_t)kpkCanData[(5u)]) & (0xFFu))
                     << ((2u) * (8u))) |
                    ((((uint64_t)kpkCanData[(6u)]) & (0xFFu))
                     << ((1u) * (8u))) |
                    ((((uint64_t)kpkCanData[(7u)]) & (0xFFu))
                     << ((0u) * (8u)));
    } else if (endianness == CAN_LITTLE_ENDIAN) {
        *pMessage = ((((uint64_t)kpkCanData[(0u)]) & (0xFFu))
                     << ((0u) * (8u))) |
        ...
        do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 354); } while (0); FAS_InfiniteLoop(); } } while (0);
    }
}
