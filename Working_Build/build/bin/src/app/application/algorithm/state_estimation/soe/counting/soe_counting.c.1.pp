...
typedef struct {
    _Bool soeInitialized;                         
    _Bool sensorEcUsed[(1u)];         
    float_t ecScalingAverage[(1u)];  
    float_t ecScalingMinimum[(1u)];  
    float_t ecScalingMaximum[(1u)];  
    uint32_t previousTimestamp
        [(1u)];  
} SOE_STATE_s;
...
static SOE_STATE_s soe_state = {
    .soeInitialized    = 0,
    .sensorEcUsed      = {(0)},
    .ecScalingAverage  = {(0.0f)},
    .ecScalingMinimum  = {(0.0f)},
    .ecScalingMaximum  = {(0.0f)},
...
static float_t SOE_GetStringSoePercentageFromEnergy(uint32_t energy_Wh) {
    ...
    if (stringEnergy_Wh >= ((40.0f) * (float_t)(((3u) * (16u)) * (1u)))) {
        stringSoe_perc = (100.0f);
    } else {
        stringSoe_perc = (100.0f) * (stringEnergy_Wh / ((40.0f) * (float_t)(((3u) * (16u)) * (1u))));
    }
...
static uint32_t SOE_GetStringEnergyFromSoePercentage(float_t stringSoe_perc) {
    ...
    if (stringSoe_perc >= (100.0f)) {
        energy_Wh = ((40.0f) * (float_t)(((3u) * (16u)) * (1u)));
    } else if (stringSoe_perc <= (0.0f)) {
        energy_Wh = (0.0f);
    } else {
        energy_Wh = ((40.0f) * (float_t)(((3u) * (16u)) * (1u))) * (stringSoe_perc / (100.0f));
    }
    return (uint32_t)energy_Wh;
}

static void SOE_RecalibrateViaLookupTable(DATA_BLOCK_SOX_s *pSoeValues) {
    do { if (!(pSoeValues != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 197); } while (0); FAS_InfiniteLoop(); } } while (0);
    DATA_BLOCK_MIN_MAX_s tableMinimumMaximumAverage = {.header.uniqueId = DATA_BLOCK_ID_MIN_MAX};

    (DATA_Read1DataBlock) (&tableMinimumMaximumAverage);
...
static float_t SOE_GetFromVoltage(int16_t voltage_mV) {
    ...
    if (!(((between_high == 0u) && (between_low == 0u)) ||        
          (between_low >= bc_stateOfEnergyLookupTableLength))) {  
...
static void SOE_SetValue(
    ...
    float_t soeAverageValue_perc,
    uint8_t stringNumber) {
    do { if (!(pSoeValues != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 252); } while (0); FAS_InfiniteLoop(); } } while (0);
    ...
    if (soe_state.sensorEcUsed[stringNumber] == 1) {
        (DATA_Read1DataBlock) (&soe_tableCurrentSensor);
...
static void SOE_CheckDatabaseSoePercentageLimits(DATA_BLOCK_SOX_s *pTableSoe, uint8_t stringNumber) {
    do { if (!(pTableSoe != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 291); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(stringNumber < (1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 292); } while (0); FAS_InfiniteLoop(); } } while (0);
...
extern void SE_InitializeStateOfEnergy(DATA_BLOCK_SOX_s *pSoeValues, _Bool ec_present, uint8_t stringNumber) {
    do { if (!(pSoeValues != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 317); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(stringNumber < (1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 318); } while (0); FAS_InfiniteLoop(); } } while (0);
    ...
    if (ec_present == 1) {
        (DATA_Read1DataBlock) (&soe_tableCurrentSensor);
        soe_state.sensorEcUsed[stringNumber] = 1;
    ...
    soe_state.soeInitialized = 1;
}

void SE_CalculateStateOfEnergy(DATA_BLOCK_SOX_s *pSoeValues) {
    do { if (!(pSoeValues != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 361); } while (0); FAS_InfiniteLoop(); } } while (0);
    _Bool continueFunction = 1;
    if (soe_state.soeInitialized == 0) {
                ...
                if (soe_state.sensorEcUsed[s] == 0) {
                    ...
                    if (soe_state.previousTimestamp[s] != timestamp) {
                        ...
                        if (timestep_s > 0.0f) {
                            ...
                            float_t deltaSOE_Wh =
                                ((((float_t)soe_tableCurrentSensor.current_mA[s] / 1000.0f) *          
                                  ((float_t)soe_tableCurrentSensor.highVoltage_mV[s][0] / 1000.0f)) /  
                                 timestep_s) /                                                         
                                3600.0f;  
                    ...
                    if (soe_state.previousTimestamp[s] != soe_tableCurrentSensor.timestampEnergyCounting[s]) {
                        ...
                        float_t deltaSoe_perc =
                            (((float_t)soe_tableCurrentSensor.energyCounter_Wh[s] / ((40.0f) * (float_t)(((3u) * (16u)) * (1u)))) *
                             (100.0f));
