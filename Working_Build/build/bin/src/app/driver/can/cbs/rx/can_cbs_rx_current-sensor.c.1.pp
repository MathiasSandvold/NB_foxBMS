...
extern uint32_t CANRX_CurrentSensor(
    ...
    const uint8_t *const kpkCanData,
    const CAN_SHIM_s *const kpkCanShim) {
    do { if (!(((message . id >= (0x521u)) && (message . id <= (0x528u))))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 86); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(message . idType == (CAN_STANDARD_IDENTIFIER_11_BIT))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 87); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(message . dlc == (6u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 88); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(kpkCanData != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 89); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(kpkCanShim != ((void *)0x0U))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 90); } while (0); FAS_InfiniteLoop(); } } while (0);
    ...
    switch (message.id) {
        static DATA_BLOCK_CELL_VOLTAGE_s mrc_tableCellVoltageBase = {
        ...
        case (0x521u):
            ...
            (DATA_Read1DataBlock) (&mrc_tableCellVoltageBase);

            
            kpkCanShim->pTableCurrentSensor->highVoltage_mV[stringNumber][0] =
                mrc_tableCellVoltageBase.packVoltage_mV[0];
            ...
            kpkCanShim->pTableCurrentSensor->highVoltage_mV[stringNumber][1] =
                mrc_tableCellVoltageBase.packVoltage_mV[0];
            ...
            kpkCanShim->pTableCurrentSensor->highVoltage_mV[stringNumber][2] =
                mrc_tableCellVoltageBase.packVoltage_mV[0];
            ...
            kpkCanShim->pTableCurrentSensor->power_W[stringNumber] =
                ((int64_t)mrc_tableCellVoltageBase.packVoltage_mV[0] * sensorSignalValue) / 1000000;
        ...
        default:
            do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 277); } while (0); FAS_InfiniteLoop(); } } while (0);
            break;  
    }

    (DATA_Write1DataBlock) (kpkCanShim->pTableCurrentSensor);
    return 0;
}
