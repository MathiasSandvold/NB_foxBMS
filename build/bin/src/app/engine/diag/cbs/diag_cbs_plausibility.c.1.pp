...
extern void DIAG_ErrorPlausibility(
    ...
    if (diagId == DIAG_ID_PLAUSIBILITY_PACK_VOLTAGE) {
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->plausibilityCheckPackVoltageError[stringNumber] = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->plausibilityCheckPackVoltageError[stringNumber] = 1;
        }
    }
}
...
extern void DIAG_PlausibilityCheck(
    ...
    const DIAG_DATABASE_SHIM_s *const kpkDiagShim,
    uint32_t stringNumber) {
    do { if (!(diagId < DIAG_ID_MAX)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 97); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!((event == DIAG_EVENT_OK) || (event == DIAG_EVENT_NOT_OK) || (event == DIAG_EVENT_RESET))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 98); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(kpkDiagShim != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 99); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(stringNumber < (1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 100); } while (0); FAS_InfiniteLoop(); } } while (0);

    if (diagId == DIAG_ID_PLAUSIBILITY_CELL_VOLTAGE) {
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->plausibilityCheckCellVoltageError[stringNumber] = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->plausibilityCheckCellVoltageError[stringNumber] = 1;
        }
    } else if (diagId == DIAG_ID_PLAUSIBILITY_CELL_TEMP) {
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->plausibilityCheckCellTemperatureError[stringNumber] = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->plausibilityCheckCellTemperatureError[stringNumber] = 1;
        }
    } else if (diagId == DIAG_ID_PLAUSIBILITY_CELL_VOLTAGE_SPREAD) {
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->plausibilityCheckCellVoltageSpreadError[stringNumber] = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->plausibilityCheckCellVoltageSpreadError[stringNumber] = 1;
        }
    } else if (diagId == DIAG_ID_PLAUSIBILITY_CELL_TEMPERATURE_SPREAD) {
        if (event == DIAG_EVENT_RESET) {
            kpkDiagShim->pTableError->plausibilityCheckCellTemperatureSpreadError[stringNumber] = 0;
        }
        if (event == DIAG_EVENT_NOT_OK) {
            kpkDiagShim->pTableError->plausibilityCheckCellTemperatureSpreadError[stringNumber] = 1;
        }
    } else {
