...
extern uint32_t ABC_ArrayAverage(uint8_t *pArray, uint8_t arrayLength, uint8_t additionalParameter) {
    do { if (!(pArray != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 76); } while (0); FAS_InfiniteLoop(); } } while (0);
    do { if (!(arrayLength > 0u)) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 77); } while (0); FAS_InfiniteLoop(); } } while (0);
