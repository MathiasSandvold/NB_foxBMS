    ...
    switch (diagId) {
        case DIAG_ID_CURRENT_SENSOR_POWER_MEASUREMENT_TIMEOUT:
            if (event == DIAG_EVENT_RESET) {
                kpkDiagShim->pTableError->currentSensorPowerTimeoutError[stringNumber] = 0;
            } else if (event == DIAG_EVENT_NOT_OK) {
                kpkDiagShim->pTableError->currentSensorPowerTimeoutError[stringNumber] = 1;
            } else {
        ...
        case DIAG_ID_POWER_MEASUREMENT_ERROR:
            if (event == DIAG_EVENT_RESET) {
                kpkDiagShim->pTableError->powerMeasurementInvalidError[stringNumber] = 0;
            } else if (event == DIAG_EVENT_NOT_OK) {
                kpkDiagShim->pTableError->powerMeasurementInvalidError[stringNumber] = 1;
            } else {
        ...
        default:
            do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 104); } while (0); FAS_InfiniteLoop(); } } while (0);
            break;  
    }
}
