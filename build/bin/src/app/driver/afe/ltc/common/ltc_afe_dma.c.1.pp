...
extern _Bool AFE_IsTransmitOngoing(LTC_STATE_s *pLtcState) {
    do { if (!(pLtcState != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 78); } while (0); FAS_InfiniteLoop(); } } while (0);
    return pLtcState->transmit_ongoing;
}

extern void AFE_SetTransmitOngoing(LTC_STATE_s *pLtcState) {
    do { if (!(pLtcState != ((void *)(0u)))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 83); } while (0); FAS_InfiniteLoop(); } } while (0);
    pLtcState->transmit_ongoing = 1;
}

 
void AFE_DmaCallback(uint8_t spiIndex) {
    if (spiIndex == 3u) {
        ltc_stateBase.transmit_ongoing = 0;
    } else {
        do { if (!((0u == 1u))) { do { uint32_t *pc = (uint32_t *)__curpc(); FAS_StoreAssertLocation(pc, 92); } while (0); FAS_InfiniteLoop(); } } while (0);
    }
}
